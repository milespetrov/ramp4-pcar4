...
import "tiny-emitter";
import { F as M, h as A, G as O, b as R } from "./main-DD4M6ybf.js";
...
import { _ as N } from "./screen.vue_vue_type_script_setup_true_lang-lTihFDsT.js";
const l = 1200, o = {
  TOP: 40,
  RIGHT: 40,
  BOTTOM: 40,
...
class B extends M {
  ...
  async make(t, e) {
    ...
    const n = f.componentSelectedState, u = this.getSubFixture("export-title"), $ = this.getSubFixture("export-map"), v = this.getSubFixture("export-scalebar"), T = this.getSubFixture("export-northarrow"), w = this.getSubFixture("export-legend"), F = this.getSubFixture("export-footnote"), b = this.getSubFixture("export-timestamp");
    let s, a, x, m, d, p, g;
    if (n.title && u && (s = await u.make({
      /* text: '😸🤖🧙‍♂️🤦‍♀️🎶', */
      ...
      width: e,
      textAlign: "center"
    }), this.options.runningHeight += s.height + 40, r.title = s), n.map && $ && (a = await $.make({
      top: this.options.runningHeight
    }), s && (s.left = a.width / 2, s.originX = "center"), this.options.runningHeight += a.height + 40, r.map = a), !a && s && (s.width = l), this.options.scale = e / ((a?.width ?? l) + o.LEFT + o.RIGHT), n.mapElements && v && (x = await v.make({
      top: this.options.runningHeight,
      left: 0
    }), this.options.runningHeight += x.height + 40, r.scaleBar = x, T && (m = await T.make({
      top: x.top,
      left: e / this.options.scale
    }), m.top += m.height / 2 - 20, m.left += -m.width * 2, r.northArrow = m)), n.legend && w && (d = await w.make({
      width: w.config?.columnWidth ?? a?.width ?? l
    }), d.top = this.options.runningHeight, this.options.runningHeight += d.height, r.legend = d), n.timestamp && b && (g = await b.make({
      top: this.options.runningHeight + 40,
      width: e * 1.5
      // Magic number 1.5 prevents unnecessary linebreaks when there's still space
    }), this.options.runningHeight += !n.footnote || !F ? g.height + 40 : g.height + 20, r.timestamp = g), n.footnote && F && (p = await F.make({
    ...
    }), n.timestamp && b ? e - r.timestamp.getMinWidth() <= p.getMinWidth() + 30 ? (p.top += 30, p.left = 0, p.originX = "left", this.options.runningHeight += 20) : p.left += -p.width * 2 : (p.top += 20, p.left += -p.width * 2, this.options.runningHeight += 20), this.options.runningHeight += p.height, r.footnote = p), this.customRendererFunc) {
...
class Jt extends B {
  ...
  async needed() {
    const t = (await import("./index-BWIRemqL.js")).default, e = (await import("./index-C-gedqHW.js")).default, i = (await import("./index-B7V3WDL9.js")).default, f = (await import("./index-DYiC4ZXt.js")).default, r = (await import("./index-BUXFhnxi.js")).default, n = (await import("./index-BEipeVed.js")).default, u = (await import("./index-tEl5zavN.js")).default;
    this.$iApi.fixture.add("export-title", t), this.$iApi.fixture.add("export-map", e), this.$iApi.fixture.add("export-legend", i), this.$iApi.fixture.add("export-northarrow", f), this.$iApi.fixture.add("export-scalebar", r), this.$iApi.fixture.add("export-timestamp", n), this.$iApi.fixture.add("export-footnote", u);
  }
  added() {
