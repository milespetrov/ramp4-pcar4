import "tiny-emitter";
import { z as O, L as I, B as d, x as P, D as R, n as G, H as z, F, G as l, _ as T } from "./main-DD4M6ybf.js";
...
import { markRaw as E, defineComponent as N, inject as D, ref as f, computed as v, reactive as x, onMounted as W, onBeforeUnmount as j, resolveDirective as q, openBlock as y, createElementBlock as g, createElementVNode as h, normalizeStyle as S, normalizeClass as U, withDirectives as X, unref as L } from "vue";
import "pinia";
...
const K = { class: "relative h-full w-full overflow-hidden" }, Q = { class: "absolute h-30 w-30 top-0 right-0" }, ee = ["content", "aria-label"], te = /* @__PURE__ */ N({
  __name: "overviewmap",
  setup(u) {
    const e = d(), { t: i } = Y(), r = D("iApi"), t = G(), o = f(), p = v(() => t.activeBasemapConfig), _ = v(() => e.mapConfig), b = v(() => e.basemaps), A = v(() => e.startMinimized);
    let a = x(new Z(r));
    ...
    W(() => {
      r.geo.map.viewPromise.then(async () => {
        ...
        let m = a.updateOverview(r.geo.map.getExtent());
        c.push(
          r.event.on(
            l.MAP_EXTENTCHANGE,
            H(100, (s) => {
      ...
      c.forEach((m) => r.event.off(m)), a.destroyMap();
    });
    const $ = async (m) => {
      M.value = !n.value && await a.cursorHitTest(m);
    }, B = () => ({
