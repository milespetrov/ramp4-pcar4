...
import { t as D, G as q } from "./main-DD4M6ybf.js";
import "pinia";
...
`, J = { key: 0 }, K = { class: "flex justify-center" }, Q = {
  ...
  setup(r) {
    const o = D(), { t: s } = G(), u = H("iApi"), M = O(), a = r, f = T(() => o.status), v = T(() => o.response), b = O(!1), x = C({}), y = C([]), S = C([]);
    j(() => {
      _(), y.push(
    ...
    const _ = () => {
      b.value = a.payload.layer !== void 0 && !a.payload.layer.isRemoved, a.payload.type === "xml" ? I(a.payload.url, []).then((t) => {
        if (o.status = "success", t !== null) {
          ...
          e.appendChild(p(`${t.firstElementChild.outerHTML}`)), (a.payload.catalogueUrl || a.payload.url) && e.appendChild(
            p(`<h5 class="text-xl font-bold mb-3">${s("metadata.xslt.metadata")}</h5>`)
      ...
      return n = n.replace(/\{\{([\w.]+)\}\}/g, (m, d) => s(d)), x[t] ? Promise.resolve($(x[t], n, e)) : w(t).then((m) => (x[t] = m.response, $(x[t], n, e)));
    }, $ = (t, e, n) => {
      let m = null;
      if (window.XSLTProcessor) {
        const d = new window.XSLTProcessor(), L = new DOMParser(), N = L.parseFromString(t, "text/xml"), E = L.parseFromString(e, "text/xml");
        d.importStylesheet(E), n && n.forEach((P) => d.setParameter("", P.key, P.value || "")), m = d.transformToFragment(N, document);
      }
      return m;
    }, w = (t) => new Promise((e) => {
    ...
    return (t, e) => {
      ...
      return l(), F(n, {
        ...
        header: k(() => [
          B(c(g(s)("metadata.title")) + ": " + c(r.payload.layerName), 1)
        ]),
        content: k(() => [
          b.value ? (l(), i("div", J, [
            h("div", K, [
              f.value == "loading" ? (l(), i("div", Q, c(g(s)("metadata.loading")), 1)) : r.payload.type === "xml" && f.value == "success" ? (l(), i("div", {
                key: 1,
                innerHTML: v.value,
                class: "flex flex-col justify-center xml-content"
              }, null, 8, W)) : (r.payload.type === "html" || r.payload.type === "md") && f.value == "success" ? (l(), i("div", {
                key: 2,
                innerHTML: v.value,
                class: "flex flex-col justify-center max-w-full metadata-view"
              }, null, 8, Z)) : (l(), i("div", tt, [
                e[0] || (e[0] = h("img", { src: "https://i.imgur.com/fA5EqV6.png" }, null, -1)),
                h("span", et, c(g(s)("metadata.error")), 1)
              ]))
            ])
          ])) : (l(), i("div", nt, [
            h("span", null, c(g(s)("metadata.label.no.layer")), 1)
          ]))
        ]),
        _: 1
      }, 8, ["panel"]);
