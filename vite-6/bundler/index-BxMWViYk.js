import "tiny-emitter";
import { F as h, k as g, b as u } from "./main-DD4M6ybf.js";
...
class x extends h {
   ...
   */
  toggleGrid(r, i) {
    const t = this.gridStore.getGridId(r);
    if (!t) {
    ...
    this.gridStore.currentId = t ?? r;
    const o = this.$iApi.panel.get("grid");
    if (i === !1) {
      o.close();
      return;
    }
    !o.isOpen || !o.isVisible ? this.$iApi.panel.open("grid") : l !== r || i === !0 ? o.show({ screen: "grid-screen", props: { key: r } }) : o.close();
  }
  /**
  ...
  _parseConfig(r) {
    ...
    r && r.mergeGrids && r.mergeGrids.forEach((t) => {
      const l = [], { gridId: o, layers: s, fieldMap: m, options: c } = t;
      s.forEach((e) => {
        e.sublayers ? e.sublayers?.forEach((a) => {
          l.push(`${e.layerId}-${a}`), delete i[`${e.layerId}-${a}`];
        }) : (l.push(e.layerId), delete i[e.layerId]);
      });
      const p = {};
      m?.forEach((e) => {
        e.sources.forEach((a) => {
...
class Tr extends x {
    ...
    this.$iApi.panel.register(
      {
        grid: {
          screens: {
            "grid-screen": () => b(import("./screen-B3nDbsYd.js"))
          },
          style: {
            width: "450px"
          },
  ...
  removed() {
    this.$iApi.fixture.exists("appbar") && u(this.$vApp.$pinia).removeButton("grid"), g(this.$vApp.$pinia).$reset(), this.$iApi.panel.remove("grid");
  }
}
export {
  Tr as default
