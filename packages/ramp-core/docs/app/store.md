...
Fixture modules can be added in the fixture's `added` function using `this.$vm.$store.registerModule(name, storeCreationFunction());`
...
Instead of having to do `this.$vm.$store....` every time we want to use anything we have some nice decorators from `vuex-pathify` to make it simple.
