...
import "tiny-emitter";
import { G as re, L as be, F as De } from "./main-Ckr7Mm0x.js";
...
}, N = "RampDrawGraphicsLayer", xe = /* @__PURE__ */ de({
  __name: "draw",
  setup(I) {
    const { t: r } = ke(), k = (e) => r(e ? `draw.${e}` : "draw.unknown"), i = ue("iApi"), t = Y();
    let n = null;
    const S = me("sketchEl");
    let F = null, u = null, d = null, V = null, x = !1, v = null, y = [];
    const J = ye([]);
    async function ne() {
    ...
    const ae = async () => {
      ...
      if (!(!document.activeElement || !o?.contains(document.activeElement)))
        switch (e.key) {
          ...
          case "Backspace":
            ...
            )) : x && y.length === 1 ? (e.preventDefault(), v && u?.remove(v), v = null, y = [], x = !1, i.updateAlert(r("draw.multiPoint.canceled"))) : d && (e.preventDefault(), n?.delete(), u?.remove(d), typeof t.removeGraphic == "function" && t.removeGraphic(d.attributes.id), d = null, T(void 0), i.updateAlert(r("draw.graphic.deleted")));
            break;
          case "Escape":
            t.setActiveTool(null), n?.cancel(), d = null, T(void 0), t.clearSelection(), i.updateAlert(r("draw.tool.canceled"));
            break;
        }
    }, X = (e) => {
      const o = i.geo.map.esriView.container;
        ...
        if (!p && !m) {
          ...
          if (h !== 0) {
            const D = Math.cos(h), R = Math.sin(h), A = f.paths.map(
              (G) => G.map(([H, K]) => {
                const C = H - l.x, q = K - l.y, j = C * D - q * R, B = C * R + q * D;
                return [l.x + j, l.y + B];
              })
            );
            g = new M({
      ...
      if (g) {
        d.geometry = g, d.set("geometry", g), T(d), d.attributes?.id && t.updateGraphicGeometry(d.attributes.id, g);
      ...
      c && t.activeTool === "edit" ? n?.triggerUpdate([c.graphic]) : (n?.cancel(), d = null, t.clearSelection(), T());
    }, ce = async () => {
      await i.geo.map.viewPromise, i.geo.layer.getLayer(N) ? u = i.geo.layer.getLayer(N).esriLayer : (u = i.geo.layer.createLayer({
        id: N,
      ...
      }), i.geo.map.esriView.ui.add(S.value, "bottom-right"), S.value?.addEventListener("arcgisCreate", (e) => ee(e.detail)), S.value?.addEventListener("arcgisUpdate", (e) => pe(e.detail)), F = {
      ...
      n && (i.geo.map.esriView && i.geo.map.esriView.ui.remove(n), F && F.remove(), n.destroy()), document.removeEventListener("keydown", Q), document.removeEventListener("keydown", X, { capture: !0 }), x = !1, v = null, y = [], n = null, u = null;
    }, ee = (e) => {
      if (e.state === "complete") {
      ...
      if (o)
        ...
        } else e.state === "active" ? T(o) : e.state === "complete" && o.attributes?.id && (t.updateGraphicGeometry(o.attributes.id, o.geometry), i.updateAlert(r("draw.graphic.updated")));
    };
    return we(() => {
      ne(), ce(), J.push(
      ...
      () => t.activeTool,
      (e) => {
        n.cancel(), T(), v = null, y = [], x = !1, e && e != "edit" && n.create(e);
      }
    ), ge(() => {
...
class mt extends Te {
  async init() {
    ...
    this.$vApp.$el.getElementsByClassName("inner-shell")[0].appendChild(r.childNodes[0]), Object.entries(Pe).forEach((t) => this.$iApi.$i18n.mergeLocaleMessage(...t)), this._parseConfig(this.config), Y(this.$vApp.$pinia).supportedTypes.forEach((t) => {
      const n = `${t.type}-icon`;
      this.$iApi.component(n, ve(Ae(/* @__PURE__ */ Object.assign({ "./icons/circle-icon.vue": () => import("./circle-icon-C_UshF2r.js"), "./icons/edit-icon.vue": () => import("./edit-icon-DBKCtjvR.js"), "./icons/point-icon.vue": () => import("./point-icon-knvC2_hP.js"), "./icons/polygon-icon.vue": () => import("./polygon-icon-ZuT2F5fe.js"), "./icons/polyline-icon.vue": () => import("./polyline-icon-CpTJEq3K.js"), "./icons/rectangle-icon.vue": () => import("./rectangle-icon-C6aBBhwl.js") }), `./icons/${t.type}-icon.vue`, 3)));
    });
  }
  async added() {
