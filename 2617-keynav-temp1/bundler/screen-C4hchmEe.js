...
import "tiny-emitter";
import { a as fe, _ as ge, G as Q, W as Ft, X as $t, L as Tt, Y as Rt, Z as tt, $ as re, i as lt, a0 as Ye, a1 as Le, a2 as St } from "./main-4ftHFA3j.js";
...
const Zt = { class: "h-full flex items-center justify-center" }, Qt = ["placeholder", "aria-label", "disabled"], Xt = ["placeholder", "aria-label", "disabled"], Jt = {
  ...
  props: ["params"],
  setup(_) {
    const n = C("none"), e = _, a = oe("iApi"), l = $t(), s = C(), { t: i } = N(), u = U(() => {
  ...
  props: ["params"],
  setup(_) {
    const n = fe(), e = oe("iApi"), { t: a } = N(), l = C(), s = C(), i = C(!1), u = _, m = U(() => n.mobileView), b = () => {
      s.value?.textContent && (i.value = !0, l.value?._tippy.show(), navigator.clipboard.writeText(s.value?.textContent), setTimeout(() => {
        i.value = !1;
      }, 2e3));
    }, w = U(() => u.params.type === "number" ? u.params.value == null ? "" : e.ui.formatNumber(u.params.value) : u.params.type === "date" ? u.params.value == null ? "" : new Date(u.params.value).toISOString().slice(0, 10) : u.params.type === "string" ? !u.params.value || /<a[^>]*>[^<]+<\/a>/g.test(u.params.value) ? u.params.value : Dt(u.params.value, {
      target: "_blank",
      validate: {
        url: (L) => /^https?:\/\//.test(L)
        // only links that begin with a protocol will be hyperlinked
      }
    }) : ""), y = U(() => /<a[^>]*>[^<]+<\/a>/g.test(u.params.value) || /(http(s)?:\/\/.*)/g.test(u.params.value));
    return X(() => {
  ...
  setup(_) {
    const n = [re.OID, re.DOUBLE, re.SINGLE, re.INTEGER], e = oe("iApi"), a = lt(), l = fe(), s = U(() => l.mobileView), i = C(!s.value), u = C(), m = Mt("gridContainer"), { t: b, locale: w } = N(), y = () => At()?.proxy?.$forceUpdate(), L = _, c = C({
      id: "dummy",
      layerIds: [],
      state: new Gt(),
      fieldMap: {}
    ...
    return X(() => {
      ...
      const v = Pe("dropdown-menu"), p = O("truncate"), d = O("tippy");
      return E(), k("div", {
        ...
        F(o("div", null, [
          o("p", Ul, $(h(b)("grid.splash.error")), 1)
        ], 512), [
          [ae, te.value]
        ]),
        F(o("div", Ol, [
