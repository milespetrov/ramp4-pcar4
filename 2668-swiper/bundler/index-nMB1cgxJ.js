import { defineComponent as $, inject as A, useTemplateRef as B, ref as u, computed as g, onBeforeMount as T, onMounted as M, onBeforeUnmount as R, resolveDirective as P, openBlock as k, createElementBlock as z, createElementVNode as f, normalizeStyle as H, withDirectives as N, unref as O, vModelText as F } from "vue";
import { F as _, a as V, G as h, _ as W } from "./main-CD5P0FCe.js";
import "pinia";
import "vue-i18n";
import "screenfull";
...
const D = { class: "swipe-container" }, G = ["id", "aria-valuenow", "aria-label", "content"], q = ["swipe-position", "view"], U = /* @__PURE__ */ $({
  ...
  setup(b) {
    const a = A("iApi"), d = V(), i = B("slider"), p = u(45), v = u(0), c = u(0), y = u(50), L = u(), r = u(), x = g(() => {
      const n = 30 + c.value - c.value, o = i.value?.getBoundingClientRect() ? i.value?.getBoundingClientRect().right - i.value?.getBoundingClientRect().left : 0, t = +p.value / 90, e = (o - n) * t, m = (i.value?.getBoundingClientRect().x ?? 0) + e + (n - 5.25) / 2;
      return y.value = m / a.$rootEl.clientWidth * 100 + 0.2, `${m}px`;
    }), C = g(() => `${v.value}px`), S = g(() => `${-v.value / 2 - 18}px`);
    T(() => {
      v.value = a.$rootEl?.clientHeight ?? 1200;
    });
    const w = () => {
      v.value = a.$rootEl?.clientHeight ?? v.value, c.value++, setTimeout(() => {
        c.value++;
      }, 50);
    }, s = () => {
    ...
    M(async () => {
      ...
      a.event.on(h.MAP_BASEMAPCHANGE, async (t) => {
      ...
      }), a.event.on(h.LAYER_RELOAD_END, (t) => {
    ...
    const E = async () => {
      await a.geo.map.viewPromise, L.value = a.geo.map.esriView;
      const n = ["Nature", "WFSLayer"], o = ["Water", "GeoMet"];
      n.forEach((t) => {
        a.geo.layer.awaitLayer(t, !0).then((e) => {
    ...
    return R(() => {
      ...
      return k(), z("div", D, [
        ...
        N(f("input", {
          ...
          id: "layerSlider" + O(a).$element._uid,
...
class Te extends _ {
  async added() {
    const { el: a, destroy: d } = this.mount(Y, {
      app: this.$element,
