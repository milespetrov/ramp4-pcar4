import { O as t, P as r, Q as $, eP as L, U as N, b_ as R, bv as g, jb as U, s as b, d_ as Z, dZ as J, iM as z, iN as H, dI as V, iL as k, iO as W, dM as K, dK as X, dL as Y, dJ as ee, dN as te, iP as w, iR as se, f0 as x, iQ as re, e4 as oe, dv as ie, b8 as ne, dw as I, iU as pe, iV as ae, iT as le, dQ as ue, iX as de, W as ce, iY as ye, iZ as he, i_ as fe, jc as me, dR as ge, j0 as ve, dX as Se } from "./main-n1d2W0Ts.js";
import { R as Ce, x as F, C as O, P as Re, O as be, N as we, q as xe, v as Ie, k as Fe } from "./ogcFeatureUtils-3ZBb8Txr.js";
let d = class extends L {
  ...
  async _loadOGCServices(e, o) {
    const i = o != null ? o.signal : null, { apiKey: n, collectionId: l, customParameters: a, fields: p, geometryType: T, hasZ: D, objectIdField: j, timeInfo: P, url: _ } = e, E = { fields: p?.map((u) => u.toJSON()), geometryType: U.toJSON(T), hasZ: D ?? !1, objectIdField: j, timeInfo: P?.toJSON() }, c = { apiKey: n, customParameters: a, signal: i }, h = await Re(_, c), [v, S] = await Promise.all([be(h, c), we(h, c)]);
    if (!this._conformsToType(v, "http://www.opengis.net/spec/ogcapi-features-1/1.0/conf/geojson")) throw new b("ogc-feature-layer:no-geojson-support", "Server does not support geojson");
    const y = S.collections.find(({ id: u }) => u === l);
    if (!y) throw new b("ogc-feature-layer:collection-not-found", "Server does not contain the named collection");
    const q = this._conformsToType(v, "http://www.opengis.net/spec/ogcapi-features-1/1.0/conf/oas30") ? await xe(h, c) : null, C = await Ie(y, E, c), Q = this._getMaxRecordCount(q), A = this._getCapabilities(C.hasZ, Q), G = this._getStorageSpatialReference(y).toJSON(), M = this._getSupportedSpatialReferences(y, S), B = new RegExp(`^${R(Fe)}`, "i"), f = {};
    for (const u of M) {
      const m = O(u);
      m != null && (f[m] || (f[m] = u.replace(B, "")));
    }
...
const Oe = ve();
let s = class extends Z(J(z(H(V(k(W(K(X(Y(ee(te(Se)))))))))))) {
  constructor(e) {
  ...
  set renderer(e) {
    w(e, this.fieldsIndex), this._set("renderer", e);
  }
  on(e, o) {
    return super.on(e, o);
  }
  ...
  async _fetchService(e) {
    await this.source.load(e), this.read(this.source.featureDefinition, { origin: "service" }), w(this.renderer, this.fieldsIndex), re(this.timeInfo, this.fieldsIndex);
  }
  _getLayerDomain(e) {
...
//# sourceMappingURL=OGCFeatureLayer-BKjhP0gG.js.map
