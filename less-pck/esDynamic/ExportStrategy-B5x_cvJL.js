import { aR as E, N, O as d, P as l, Q as H, dH as $, eG as P, dm as w, aS as q, a1 as T, dE as B, eq as z, f8 as C, F as I } from "./main-n1d2W0Ts.js";
import { b as O } from "./Bitmap-biJly5ra.js";
const U = Math.PI / 180;
function k(e) {
  return e * U;
}
function A(e, i) {
  const s = k(i.rotation), t = Math.abs(Math.cos(s)), r = Math.abs(Math.sin(s)), [o, n] = i.size;
...
let h = class extends $ {
  constructor(e) {
    super(e), this._imagePromise = null, this.bitmaps = [], this.hidpi = y.hidpi, this.imageMaxWidth = y.imageMaxWidth, this.imageMaxHeight = y.imageMaxHeight, this.imageRotationSupported = y.imageRotationSupported, this.imageNormalizationSupported = y.imageNormalizationSupported, this.update = P(async (i, s) => {
      if (w(s), !i.stationary || this.destroyed) return;
      const t = i.state, r = q(t.spatialReference), o = this.hidpi ? i.pixelRatio : 1, n = t.worldScreenWidth > 0, a = n && this.imageNormalizationSupported && t.worldScreenWidth < t.size[0], c = Math.round((this.imageMaxWidth ?? 0) / o), x = Math.round((this.imageMaxHeight ?? 0) / o);
      a ? (u[0] = t.worldScreenWidth, u[1] = t.size[1]) : this.imageRotationSupported ? (u[0] = t.size[0], u[1] = t.size[1]) : A(u, t);
      const M = Math.floor(u[0]) > c || Math.floor(u[1]) > x, S = r && (t.extent.xmin < r.valid[0] || t.extent.xmax > r.valid[1]), v = !this.imageNormalizationSupported && S, f = !M && !v, W = this.imageRotationSupported ? t.rotation : 0, b = this.container.children.slice();
      if (f) {
      ...
      try {
        ...
        this.bitmaps = p;
        for (const g of b) p.includes(g) || R.push(g.fadeOut().then(() => {
          g.remove(), g.destroy();
        }));
    ...
    }, 5e3), this.updateExports = P(async (i) => {
      const s = [];
  ...
  _tiledExport(e, i, s, t) {
    ...
    if (!n) return null;
    const a = [];
    return n.forEach((c, x, M, S) => {
      _.set(c, x, M, 0), o.getTileBounds(m, _);
      const v = z(m, e.spatialReference);
      a.push(this._export(v, i, i, 0, s, t).then((f) => (S !== 0 && (_.set(c, x, M, S), o.getTileBounds(m, _), f.x = m[0], f.y = m[3]), f)));
    }), Promise.all(a);
  }
};
...
//# sourceMappingURL=ExportStrategy-B5x_cvJL.js.map
