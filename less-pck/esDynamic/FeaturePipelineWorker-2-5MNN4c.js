import { rH as Ce, N as Te, oT as dt, cP as Me, bz as $e, cR as ss, c$ as rs, nn as lt, cU as _t, rI as Q, rJ as is, bv as O, d0 as pt, d4 as U, k2 as ns, rK as as, rL as os, d5 as F, m0 as G, bo as hs, rM as us, m5 as cs, b5 as ds, b8 as ls, mU as gt, d$ as Ue, nF as We, rN as _s, c_ as ps, qA as Z, ac as m, q1 as ft, f6 as qe, Z as yt, dm as V, f0 as mt, eJ as gs, eY as fs, eX as ys, O as W, P as K, Q as bt, dH as It, S as fe, rO as ms, rP as bs, s as j, kO as Is, oz as ws, D as Ss, rQ as vs, o6 as wt, lv as Ge, rR as St, rS as xs, eH as ie, lU as se, lV as ks, mZ as vt, aP as Fs, eI as xt, rT as Ne, rU as Be, a2 as Cs, nr as Ts, f8 as Ms, F as $s, cN as qs, j$ as Es, rV as Os, ag as Rs } from "./main-n1d2W0Ts.js";
import { a as k } from "./definitions-Doe0g1C2.js";
import { n as Ve, M as Ee, p as Oe, c as kt, i as Re, d as As, a as Ps, b as js, e as Ds, f as Ls } from "./AttributeStore-GRl8PvEZ.js";
import { o as zs } from "./tileUtils-DrLrf-3T.js";
import { r as Us, W as Ws } from "./MeshWriterRegistry-BuCCC4IT.js";
import { x as Gs, j as re } from "./timeSupport-BajsmNHb.js";
import { o as Ns, $ as Ft } from "./QueryEngine-CQ5A0Pnd.js";
import { $ as Ye } from "./ogcFeatureUtils-3ZBb8Txr.js";
import { t as Xe } from "./CircularArray-DaQg3PQl.js";
import { createConnection as Bs } from "./createConnection-BZqO1-An.js";
import { d as Vs } from "./UpdateTracking2D-BihgDBEV.js";
let Ys = class {
  constructor(e) {
...
let Xs = class {
  ...
  serialize(e) {
    ...
    return t.message.push({ tileId: this.tileKey.id, ...s.message }), t.transferList.push(...s.transferList), this._meshes.neighbors.forEach((r, i) => {
      const n = r.serialize(), a = 1 << i, o = de(a), h = le(a), d = zs(new Te(this.tileKey), o, h, e);
      t.message.push({ tileId: d.id, ...n.message }), t.transferList.push(...n.transferList);
    }), t;
  }
  _addOverlap(e, t, s, r) {
  ...
  _copyIntoNeighbors() {
    for (let e = 0; e < 8; e++) {
      ...
      if (this._currentRecordOverlaps & t) {
        ...
        const s = this._meshes.neighbors[e], r = -de(t) * k, i = -le(t) * k;
  ...
  static async fromMatcherSchema(e, t) {
    ...
    return i.setBackgroundFill(a), await Promise.all(t.map.map(async (o, h) => {
      const d = await e.createMeshWriters(o.symbol);
      o.value === "<Null>" ? i.setNullResult(d) : i.add(o.value, d, h + 1);
    })), i;
  }
  setNullResult(e) {
    this._nullResult = e;
...
let er = class Et extends D {
  ...
  constructor(e) {
    this._outstandingMessages = [], this._queue = new dt({ concurrency: e.concurrency, process: (t) => e.process(t) });
  }
  async push(e) {
  ...
  *featuresWorldSpace() {
  ...
  displayMap(e, t, s) {
    ...
    for (const n of this._sendStates.values()) for (const a of n.featuresWorldSpace()) {
      const { objectId: o, displayId: h } = a, d = r.get(o);
      if (d != null) {
        const c = s(h, d, o);
        i.push(c), r.delete(o);
      }
  ...
  async *_update(e, t) {
    ...
    const o = [], h = r.tile.transform, d = r.tile.key.level;
    for (const f of i.values()) {
      let p = n.get(f.id);
      if (p) p.attributes = f.getAttributes();
      else {
        const y = f.getGeometry(this.spatialReference, h);
        p = new U(y, f.getAttributes(), null), y || (p.centroid = f.getGeometricCentroid(this.spatialReference, h)), p.objectId = f.id, p.displayId = this._attributeStore.createDisplayIdForObjectId(`${p.objectId}.${d}`), n.set(f.id, p);
      }
      o.push(p);
    }
    this.events.emit("changed"), e.done = !t.updateTracking.updating;
    const c = M.fromOptimizedFeatures(o, this._metadata, h), _ = c.getCursor(), g = e.subscription.tile.createArcadeEvaluationOptions(this._timeZone);
    for (; _.next(); ) this._attributeStore.setAttributeData(_.getDisplayId(), _, g);
    yield new me(e.subscription, c, [], e.done, {});
  }
}, Lt = class {
  constructor(e) {
    this._statistics = e;
...
let gr = class extends Yt {
  constructor(e) {
    super(e), this._cells = /* @__PURE__ */ new Map(), this._pixelsPerMapUnit = we(e.spatialReference, e.scale);
  }
  put(e) {
  ...
  putBounded(e, t, s) {
    const r = [t.xmin, t.ymin, t.xmax, t.ymax], [i, n, a, o] = r, h = Math.floor(i * this._pixelsPerMapUnit / this._options.cellSize), d = Math.floor(n * this._pixelsPerMapUnit / this._options.cellSize), c = Math.ceil(a * this._pixelsPerMapUnit / this._options.cellSize), _ = Math.ceil(o * this._pixelsPerMapUnit / this._options.cellSize);
    for (let g = d; g <= _; g++) for (let l = h; l <= c; l++) {
...
let yr = class extends Le {
  ...
  *aggregatesWorldSpace() {
  ...
  updateChunks(e, t) {
    ...
    const r = { xMin: 1 / 0, yMin: 1 / 0, xMax: -1 / 0, yMax: -1 / 0 }, i = we(this._indexOptions.spatialReference, this._scale), n = this._indexOptions.cellSize;
    for (const { subscription: a } of t) {
      const o = a.tile.bounds, h = Math.floor(o[0] * i / n), d = Math.floor(o[1] * i / n), c = Math.ceil(o[2] * i / n), _ = Math.ceil(o[3] * i / n);
      r.xMin = Math.min(r.xMin, h), r.yMin = Math.min(r.yMin, d), r.xMax = Math.max(r.xMax, c), r.yMax = Math.max(r.yMax, _);
    }
  ...
  createAggregateFeatures(e, t) {
    ...
    for (const n of this._clusters.values()) {
      ...
      const o = n.inner.getCentroidY(i), h = s.tile.lod, d = h.wrap ? h.worldSize[0] : null, c = n.inner.count === 1 ? n.inner.firstObjectId : n.inner.id, _ = n.displayId;
      if (d != null) if (d === 1) {
        const g = new F([], [a, o]), l = new U(g, n.inner.getAttributes(), null);
        l.geometry.coords[0] -= k, l.objectId = c, l.displayId = _, r.push(l);
        const f = new F([], [a, o]), p = new U(f, n.inner.getAttributes(), null);
        p.geometry.coords[0] += k, p.objectId = c, p.displayId = _, r.push(p);
      } else a > k + k / 2 ? a -= d * k : a < -k / 2 && (a += d * k);
      if (a < k + H && a >= -H && o < k + H && o >= -H) {
        const g = new F([], [a, o]), l = new U(g, n.inner.getAttributes(), null);
        l.objectId = c, l.displayId = _, r.push(l);
      }
    }
  ...
  _clusterCells(e) {
    ...
    this._clusters.clear();
    const r = this._clusterRadius * (1 / we(this._indexOptions.spatialReference, this._scale)), i = 1 + this._clusterRadius / this._indexOptions.cellSize, n = /* @__PURE__ */ new Set();
    for (const a of t) {
      ...
      const d = a.centroidXWorld, c = a.centroidYWorld;
      for (let _ = a.gridY - i; _ <= a.gridY + i; _++) for (let g = a.gridX - i; g <= a.gridX + i; g++) {
        ...
        const f = Math.abs(l.centroidXWorld - d), p = Math.abs(l.centroidYWorld - c);
  ...
  _updateAggregateStatistics(e, t) {
    ...
    for (const r of t.statistics.values()) {
      ...
      if (a) {
        const { minValue: o, maxValue: h } = a, d = Math.min(a.minValue, i), c = Math.max(a.maxValue, i);
        o === d && h === c || (a.minValue = d, a.maxValue = c, s = !0);
    ...
    for (const a of n.clusters()) {
      ...
      if (o == null) {
        if (a.inner.count === 1) {
          ...
          if (h != null) {
            const d = s(a.displayId, h, a.inner.firstObjectId);
            i.push(d), r.delete(a.inner.firstObjectId);
          }
        }
      } else {
  ...
  static fromReader(e) {
    const t = [], s = e.copy(), r = $e();
    for (; s.next(); )
      s.getBounds(r) && t.push(s.getIndex());
    const i = cs(9, (n) => (s.setIndex(n), { minX: s.getBoundsXMin(), minY: s.getBoundsYMin(), maxX: s.getBoundsXMax(), maxY: s.getBoundsYMax() }));
...
const wr = 64;
function Sr(u, e, t, s) {
  const r = [u.xmin, u.ymin, u.xmax, u.ymax], i = ds.fromExtent(ls.fromBounds(r, s)), n = re(i, s, O.WGS84, { densificationStep: e * wr });
  if (!n) return null;
  const a = gt(new F(), n, !1, !1), o = a.coords.filter((p, y) => !(y % 2)), h = a.coords.filter((p, y) => y % 2), d = Math.min(...o), c = Math.min(...h), _ = Math.max(...o), g = Math.max(...h), l = Se(d, c, t, O.WGS84), f = Se(_, g, t, O.WGS84);
...
function vr(u, e) {
  ...
  for (let n = 0; n < e; n++) {
    const a = Math.ceil((n + 1) / 2), o = Math.floor((n + 1) / 2), h = 1 - n % 2, d = 30 - (3 * a + 2 * o), c = 30 - (2 * a + 3 * o), _ = 3 * h + 2 * (1 - h), g = 2 * h + 3 * (1 - h), l = 3 * h + 7 * (1 - h) << c, f = (7 * h + 3 * (1 - h) << d & u.geohashX) >> d, p = (l & u.geohashY) >> c;
    for (let y = _ - 1; y >= 0; y--) {
...
function et(u, e, t, s, r) {
  r % 2 && (r += 1);
  let i = 0, n = 0, a = -90, o = 90, h = -180, d = 180;
  for (let c = 0; c < r / 2; c++) {
    for (let _ = 0; _ < 5; _++) {
      const g = (h + d) / 2, l = s > g ? 1 : 0;
      i |= l << 29 - (_ + 5 * c), h = (1 - l) * h + l * g, d = (1 - l) * g + l * d;
    }
    for (let _ = 0; _ < 5; _++) {
      const g = (a + o) / 2, l = t > g ? 1 : 0;
      n |= l << 29 - (_ + 5 * c), a = (1 - l) * a + l * g, o = (1 - l) * g + l * o;
    }
  }
  u[2 * e] = i, u[2 * e + 1] = n;
}
let tt = class ve extends Lt {
  ...
  find(e, t, s, r, i, n) {
    if (r >= s) return this;
    const a = 1 - r % 2, o = 3 * a + 2 * (1 - a), h = 2 * a + 3 * (1 - a), d = 30 - i - o, c = 30 - n - h, _ = ((e & 7 * a + 3 * (1 - a) << d) >> d) + ((t & 3 * a + 7 * (1 - a) << c) >> c) * (8 * a + 4 * (1 - a)), g = this.children[_];
...
class xr {
  ...
  insert(e, t, s, r, i, n, a) {
    ...
    for (; o !== null; ) {
      ...
      d = d << 3 * l + 2 * (1 - l) | y, c = c << 2 * l + 3 * (1 - l) | b, o.children[I] == null && (o.children[I] = tt.create(this._fields, d, c, h + 1), this._depth = Math.max(this._depth, h + 1), this._size += 1), h += 1, o = o.children[I];
    }
  }
  putBins(e, t) {
  ...
  getNodes(e) {
    ...
    for (; i !== null; ) {
      ...
      const h = Math.ceil((n + 1) / 2), d = Math.floor((n + 1) / 2), c = 1 - n % 2, _ = 30 - (3 * h + 2 * d), g = 30 - (2 * h + 3 * d), l = ~((1 << _) - 1), f = ~((1 << g) - 1), p = (s.xLL & l) >> _, y = (s.yLL & f) >> g, b = (s.xTR & l) >> _, I = (s.yTR & f) >> g, x = a << 3 * c + 2 * (1 - c), w = o << 2 * c + 3 * (1 - c), S = x + 8 * c + 4 * (1 - c), v = w + 4 * c + 8 * (1 - c), C = Math.max(x, p), E = Math.max(w, y), $ = Math.min(S, b), T = Math.min(v, I);
      let q = null, R = null;
      for (let A = E; A <= T; A++) for (let ce = C; ce <= $; ce++) {
        const ts = ce - x + (A - w) * (8 * c + 4 * (1 - c)), B = i.children[ts];
        B && (q || (q = B, q.next = i.next), R && (R.next = B), R = B, B.next = i.next);
      }
      i = q || i.next;
    }
    return t;
...
let kr = class extends Yt {
  ...
  putBounded(e, t, s) {
    const { geohashLevel: r, spatialReference: i } = this._options, n = Sr(t, s, r, i);
    n != null && this._tree.putBins(e, n);
  }
  _insert(e, t, s, r) {
    const { geohashLevel: i, spatialReference: n } = this._options, a = Se(t, s, i, n);
  ...
  setTransformForDisplay(e) {
    ...
    if (t == null) {
      const [c, _] = e.scale, [g, l] = e.translate;
      return this._displayTranslationX = -g / c, this._displayScaleX = 1 / c, this._displayTranslationY = l / _, this._displayScaleY = 1 / -_, void (this._displayTransform = e);
    }
    const [s, r] = t.scale, [i, n] = t.translate, [a, o] = e.scale, [h, d] = e.translate;
    if (this._displayScaleX = s / a, this._displayTranslationX = (i - h) / a, this._displayScaleY = r / o, this._displayTranslationY = (-n + d) / o, !this._isPoint && t) throw new Error("InternalError: Relative transformations not supported for non-point features");
  ...
  displayMap(e, t, s) {
    ...
    for (const n of this._source.chunks()) {
      ...
      for (; a.next(); ) {
        const o = a.getObjectId(), h = a.getDisplayId(), d = r.get(o);
        if (d != null) {
          const c = s(h, d, o);
          i.push(c), r.delete(o);
        }
  ...
  *_update(e) {
  ...
  async update(e, t, s, r, i) {
    const n = e.processor, a = Z(this._schema?.storage, n.storage), o = Z(this._schema?.mesh.properties, n.mesh.properties), h = Z(this._schema?.mesh.factory, n.mesh.factory), d = Z(this._schema?.mesh.strategy, n.mesh.strategy), c = o || h || d;
    if (!(a || c) && !r) return;
    ...
    if (await this._attributeStore.update(n.storage, g, this._source.metadata, _, t), this._strategy?.invalidateAttributeData(), !r && !c) return;
    (d || o) && await this._updateStrategy(n.mesh.strategy, _, i, n.mesh.properties.timeZone), this._updateSortKey(g, "sortKey" in n.mesh.properties ? n.mesh.properties.sortKey : null);
...
let Er = class Ht {
  static fromSchema(e, t, s) {
    ...
    return new Ht(qr(e, t, s), r, s.subtypeField, t.mutable.dataFilter.customParameters, s.geometryType, e.queryMetadata);
  }
  constructor(e, t, s, r, i, n) {
...
function Ar(u, e, t = !1) {
  const h = u.asUnsafe(), d = h.pos(), c = new Rr();
  let _ = 0, g = 0;
  const l = 1, f = 2, p = 4, y = 3;
  let b = null, I = null, x = null, w = !1;
  const S = [];
  for (; h.next(); ) switch (h.tag()) {
    ...
    case 15: {
      ...
      if (!c.exceededTransferLimit) {
        const T = c.offsets.geometry, q = c.centroid;
        T.push(0), q.push(pe), q.push(pe);
      }
      !w && c.exceededTransferLimit && (w = !0, c.offsets.attributes = t ? new Float64Array(8e3 * _) : new Uint32Array(8e3 * _));
      let $ = g * _;
      for (; h.pos() < E && h.next(); ) switch (h.tag()) {
        case l: {
          w ? c.offsets.attributes[$++] = h.pos() : c.offsets.attributes.push(h.pos());
          const T = h.getLength();
          h.skipLen(T);
          break;
        }
        case f:
          if (e) {
            const T = h.getLength(), q = h.pos() + T;
            for (; h.pos() < q && h.next(); ) switch (h.tag()) {
              case y: {
                h.getUInt32();
                const R = h.getSInt64(), A = h.getSInt64();
                c.centroid[2 * g] = R, c.centroid[2 * g + 1] = A;
                break;
              }
              default:
                h.skip();
            ...
            c.offsets.geometry[g] = h.pos();
            const T = h.getLength();
            c.vertexCount += T, h.skipLen(T);
          }
          break;
        case p: {
          const T = h.getLength(), q = h.pos() + T;
  ...
  return c.fields = new _t(S), c.featureCount = g, c.fieldCount = _, c.objectIdFieldIndex = c.fields.get(v)?.index, c.transform = x, c.displayIds = new Uint32Array(c.featureCount), c.groupIds = new Uint16Array(c.featureCount), h.move(d), c;
}
...
function jr(u) {
  try {
    const t = new ws(new Uint8Array(u), new DataView(u));
    for (; t.next(); ) {
    ...
    Ss.getLogger("esri.view.2d.layers.features.support.FeatureSetReaderPBF").error(t);
  }
  return null;
}
function Dr(u) {
...
function Lr(u) {
  const d = u.getLength(), c = u.pos() + d;
  for (; u.pos() < c && u.next(); ) switch (u.tag()) {
    case 1:
      return u.getString();
    case 2:
      return u.getFloat();
...
let Ur = class Fe extends Ee {
  ...
  _parseGeometry(e) {
    ...
    for (; r.pos() < n && r.next(); ) switch (r.tag()) {
      case 2: {
        const h = r.getUInt32(), d = r.pos() + h;
        for (; r.pos() < d; ) o.push(r.getUInt32());
        break;
      }
      case 3: {
        const h = r.getUInt32(), d = r.pos() + h;
        for (a.push(r.getSInt64()), a.push(r.getSInt64()), this.hasZ && r.getSInt64(), this.hasM && r.getSInt64(); r.pos() < d; ) a.push(r.getSInt64()), a.push(r.getSInt64()), this.hasZ && r.getSInt64(), this.hasM && r.getSInt64();
        break;
      }
      default:
        r.skip();
  ...
  _parseGeometryForDisplay(e) {
    ...
    for (; r.pos() < n && r.next(); ) switch (r.tag()) {
      ...
      case 3: {
        ...
        Is(c);
        for (const f of a) if (d + l * f > c.length) for (let p = 0; p < f; p++) r.getSInt32(), r.getSInt32(), this.hasZ && r.getSInt32(), this.hasM && r.getSInt32();
        else if (g && Pr) {
          const p = this.getAreaSimplificationThreshold(f, this._header.vertexCount);
          let y = 2, b = 1;
          const I = !1;
          let x = r.getSInt32(), w = r.getSInt32();
          c[d++] = x, c[d++] = w, this.hasZ && r.getSInt32(), this.hasM && r.getSInt32();
          let S = r.getSInt32(), v = r.getSInt32();
          for (this.hasZ && r.getSInt32(), this.hasM && r.getSInt32(); y < f; ) {
            ...
            const $ = x + S, T = w + v;
            zr(x, w, $, T, $ + C, T + E) >= p ? (_ += -0.5 * ($ - x) * (T + w), b > 1 && ge(c[d - 2], c[d - 1], S, v) ? (c[d - 2] += S, c[d - 1] += v) : (c[d++] = S, c[d++] = v, b++), x = $, w = T) : (C += S, E += v), S = C, v = E, y++;
          }
          b < 3 || I ? d -= 2 * b : (_ += -0.5 * (x + S - x) * (w + v + w), ge(c[d - 2], c[d - 1], S, v) ? (c[d - 2] += S, c[d - 1] += v, o.push(b)) : (c[d++] = S, c[d++] = v, o.push(++b)));
        } else {
          let p = 0, y = r.getSInt32(), b = r.getSInt32();
          this.hasZ && r.getSInt32(), this.hasM && r.getSInt32(), c[d++] = y, c[d++] = b, p += 1;
          for (let I = 1; I < f; I++) {
            const x = r.getSInt32(), w = r.getSInt32(), S = y + x, v = b + w;
            _ += -0.5 * (S - y) * (v + b), this.hasZ && r.getSInt32(), this.hasM && r.getSInt32(), I > 2 && ge(c[d - 2], c[d - 1], x, w) ? (c[d - 2] += x, c[d - 1] += w) : (c[d++] = x, c[d++] = w, p += 1), y = S, b = v;
          }
          o.push(p);
        }
        break;
    ...
    return this._cache.area = _, o.length ? new F(o, c) : c != null ? this._createQuantizedExtrudedGeometry(c[0], c[1]) : null;
  }
};
class ue {
  constructor(e, t) {
...
class Vr extends ue {
  async executeQuery(e, t) {
    ...
    if (e.quantizationParameters != null && !r.supportsQuantization) {
      const n = e.clone(), a = wt(n.quantizationParameters);
      ...
      return St(a, h), M.fromOptimizedFeatureSet(h, this._metadata);
    }
...
let Yr = class extends ue {
  async executeQuery(e, t) {
    if (e.quantizationParameters && !this.service.queryMetadata.supportsQuantization) {
      const r = e.clone(), i = wt(r.quantizationParameters);
      r.quantizationParameters = null;
      const n = await Ye(this.service.source, e, t);
      return St(i, n), M.fromOptimizedFeatureSet(n, this._metadata);
    }
  ...
  constructor(e, t, s, r, i) {
    super(s), this._serviceInfo = e, this._queryInfo = t, this._metadata = r, this._connection = i, this._queue = new dt({ concurrency: 16, process: async (n) => {
  ...
  async load(e) {
    ...
    try {
      for await (const i of this._fetchChunkInfos(t, e.tile, 0)) {
        const { queryJSON: n, reader: a, sourceTile: o, sourceTileDepth: h, tile: d } = i, c = new at(a, n, d, o, h, !1);
        V(t.options), this._addChunk(c);
      }
    } catch (i) {
      s = i;
    }
...
const Kr = "__esri_stream_id__", ut = "__esri_timestamp__", ct = 1e3;
class ei {
  ...
  add(e) {
    ...
    if (!this._trackIdToObservations.has(s)) {
      const n = this._purgeOptions?.maxObservations != null ? this._purgeOptions.maxObservations : ct, a = Fs(n, 0, ct);
...
class sn {
  ...
  onAttach(e) {
    ...
    this._tileInfoView = new $s(s), this._source = new ni(e.service, this._aggregateAdapter, this._subscriptions, () => this._requestUpdate(), t), this._processor = new Mr(t, this._source), this._handles = ie([fe(() => this._source.updateTracking.updating, () => {
  ...
  async queryHeatmapStatistics(e) {
    const t = Math.round(qs(e.radius));
    let s = Number.POSITIVE_INFINITY, r = Number.NEGATIVE_INFINITY;
    const i = typeof e.fieldOffset == "string", n = e.fieldOffset ?? 0, a = Array.from(this._subscriptions.values()), o = this._source.chunks(), h = t ** 2, d = 3 / (Math.PI * h), c = 2 * t, _ = Math.ceil(k / c);
    for (const g of a) {
      ...
      for (const p of o) {
        ...
        for (; b.next(); ) {
          let I = 1;
          if (e.field != null) {
            const $ = b.readAttribute(e.field);
            I = i ? -1 * +$ : +$ + n;
          }
          const x = b.readXForDisplay() / c, w = b.readYForDisplay() / c, S = Math.floor(x), v = Math.floor(w);
          if (S < 0 || v < 0 || S >= _ || v >= _) continue;
          const C = ((0.5 + S - x) * c) ** 2 + ((0.5 + v - w) * c) ** 2;
          if (C > h) continue;
          const E = I * (d * (1 - C / h) ** 2);
          f[v + S * _] += E;
        }
  ...
  async getSampleFeatures(e) {
    ...
    for (let o = 0; o < a && r.length < e.sampleSize; o++) {
      ...
      const d = n.getIntRange(0, h.getSize() - 1);
      h.setIndex(d);
      const c = h.getObjectId();
      s.has(c) || (s.add(c), r.push(h.readLegacyFeatureWorldSpace()));
    }
...
//# sourceMappingURL=FeaturePipelineWorker-2-5MNN4c.js.map
