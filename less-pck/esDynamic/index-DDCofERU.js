import { bW as m, kW as h, kX as g, hk as p, jX as u } from "./main-n1d2W0Ts.js";
...
const r = "details";
class y extends m {
  detailsStore = h(this.$vApp.$pinia);
  get config() {
    return super.config;
  }
  /**
  ...
  toggleFeature(e, t) {
    ...
    const i = this.$iApi.geo.layer.getLayer(e.uid), a = `${e.uid}-${// see https://github.com/ramp4-pcar4/ramp4-pcar4/issues/1767 for the reasoning behind this
    i?.supportsFeatures ? e.data[i?.oidField ?? ""] : JSON.stringify(e.data)}`;
    if (s.isOpen && a === this.detailsStore.currentFeatureId && t !== !0) {
      s.close(), this.detailsStore.currentFeatureId = void 0;
      return;
    }
    ...
    const l = {
      items: [g.makeRawItem(e.format, e.data)],
      uid: e.uid,
  ...
  async hilightDetailsItems(e, t) {
    ...
    if (i) {
      ...
      const o = await this.getHilightGraphics(s, t);
  ...
  async removeDetailsHilight() {
    ...
    if (e) {
      ...
      const t = await e.getGraphicsByKey(r);
  ...
  async reloadDetailsHilight(e, t) {
    const s = e instanceof Array ? e : [e], i = this.$iApi.fixture.get("hilight");
    if (i) {
      const a = await this.getHilightGraphics(s, t);
      i.reloadHilight(a);
    }
  }
  /**
  ...
  async getHilightGraphics(e, t) {
    const s = this.$iApi.geo.layer.getLayer(t), i = this.$iApi.fixture.get("hilight"), a = [];
    return s && await Promise.all(
      e.map(async (l) => {
        ...
        n.id = i.constructGraphicKey(r, t, o), a.push(n);
      })
    ), a;
  }
  /**
  ...
  hasHilighter() {
    const e = this.$iApi.fixture.get("hilight");
    return e && e.hilightMode.mode !== c.NONE;
  }
}
...
class S extends y {
  async added() {
    this.$iApi.panel.register(
      {
        details: {
          screens: {
            "details-screen": () => p(import("./details-screen-3URJTWez.js"))
          },
          style: {
            width: "425px"
          },
    ...
    this.removed = () => {
      e(), this.$iApi.panel.remove("details"), this.$iApi.fixture.exists("appbar") && u(this.$vApp.$pinia).removeButton("details"), h(this.$vApp.$pinia).$reset();
    };
  }
}
export {
...
//# sourceMappingURL=index-DDCofERU.js.map
