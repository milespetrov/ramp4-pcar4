import { s as j, dc as q, by as Y, a2 as x, cV as ee, cU as L, bg as ne, bv as k, dd as te, c_ as ie, cZ as ae, de as E, cT as re, I as A, D as se, df as oe } from "./main-n1d2W0Ts.js";
import { E as le, I as ce, N as ue } from "./geojson-CqcYYCeT.js";
import { o as de } from "./clientSideDefaults-D1T7exSR.js";
import { p as fe } from "./sourceUtils-BFS5QS-E.js";
const G = () => se.getLogger("esri.layers.ogc.ogcFeatureUtils"), J = "startindex", me = /* @__PURE__ */ new Set([J, "offset"]), _ = "http://www.opengis.net/def/crs/", xe = `${_}OGC/1.3/CRS84`;
var l;
async function $e(e, t, n = {}, i = 5) {
  ...
  const { apiKey: d, customParameters: g, signal: p } = n, u = q(o.href, e.landingPage.url), I = { limit: i, ...g, token: d }, $ = Y(u, I), R = { accept: l.geojson }, { data: v } = await x($, { signal: p, headers: R }), S = Ie($, i, v.links) ?? J;
  le(v);
  const f = ce(v, { geometryType: t.geometryType }), h = t.fields || f.fields || [], D = t.hasZ != null ? t.hasZ : f.hasZ, y = f.geometryType, w = t.objectIdField || f.objectIdFieldName || "OBJECTID";
  let s = t.timeInfo;
  ...
  if (s) {
    ...
    if (s.trackIdField) {
      ...
      c ? s.trackIdField = c.name : (s.trackIdField = null, G().warn({ name: "ogc-feature-layer:invalid-timeInfo-trackIdField", message: "trackIdField is missing", details: { timeInfo: s } }));
    }
    s.timeReference ||= { timeZoneIANA: ne }, s.startTimeField || s.endTimeField || (G().warn({ name: "ogc-feature-layer:invalid-timeInfo", message: "startTimeField and endTimeField are missing", details: { timeInfo: s } }), s = void 0);
  }
  return { drawingInfo: y ? de(y) : null, extent: he(e), geometryType: y, fields: h, hasZ: !!D, objectIdField: w, paginationParameter: S, timeInfo: s };
}
...
async function ge(e, t, n) {
  ...
  const { geometry: R, num: v, start: S, timeExtent: f, where: h } = t;
  if (t.objectIds) throw new j("ogc-feature-layer:query-by-objectids-not-supported", "Queries with object ids are not supported");
  const D = k.fromJSON(u), y = t.outSpatialReference ?? D, w = y.isWGS84 ? null : U(y, I), s = be(R, I), N = ye(f), r = we(h), c = v ?? (S == null ? d : 10), z = S === 0 ? void 0 : S, { fields: M, geometryType: C, hasZ: W, objectIdField: P, paginationParameter: V } = o, B = q($.href, a), { data: K } = await x(B, { ...n, query: { ...p, ...s, crs: w, datetime: N, query: r, limit: c, [V]: z, token: g }, headers: { accept: l.geojson } }), O = ue(K, { geometryType: C, hasZ: W, objectIdField: P }), Q = O.length === c && !!m(K.links ?? [], "next", l.geojson), H = new L(M);
  for (const b of O) {
    const T = {};
  ...
  return F.exceededTransferLimit = Q, F.features = O, F.fields = M, F.geometryType = C, F.hasZ = W, F.objectIdFieldName = P, F.spatialReference = X, F;
}
function pe(e) {
...
//# sourceMappingURL=ogcFeatureUtils-3ZBb8Txr.js.map
