import { s as k, dm as $, nR as ie, aj as re, fo as L, di as Y, aF as ne, aH as ae, aI as M, a8 as j, cf as oe, dg as v, o5 as Q, ge as ce, c4 as he, D as fe, eZ as ue } from "./main-n1d2W0Ts.js";
import { N as le, T as de } from "./quat-7UY_HF03.js";
import { o as me, r as _e, e as pe } from "./quatf64-DxbQqBtW.js";
import { m as ge } from "./Mesh-DYGRp2Vu.js";
import { M as we } from "./vertexSpaceConversion-DpZAFo-9.js";
import { C as _, D as b } from "./enums-DDkmfb-t.js";
import { r as U } from "./resourceUtils-BCkesTE3.js";
function Ae(i) {
  const e = q(i);
...
async function J(i, e) {
  ...
  if ($(e), !r) throw new k("imageToArrayBuffer", "Failed to encode image");
...
let I = class N {
  constructor(e, t) {
    ...
    const s = Re(e);
    if (this._length += K(s.byteLength, 4), t && (this._length += N.CHUNK_HEADER_SIZE, this._length += t.byteLength, t.byteLength % 4)) throw new Error("Expected BIN chunk length to be divisible by 4 at this point");
...
function Ie(i, e) {
  ...
  for (let a = 0; a < n; ++a) {
    const o = 3 * t[3 * a], c = 3 * t[3 * a + 1], h = 3 * t[3 * a + 2], u = L(Me, s[o], s[o + 1], s[o + 2]), p = L(Se, s[c], s[c + 1], s[c + 2]), l = L(Le, s[h], s[h + 1], s[h + 2]), w = Y(p, p, u), d = Y(l, l, u), m = ne(w, w, d);
...
class Ce {
  ...
  _addScenes(e) {
    ...
    s && (t.binChunkBuffer = new X(this.gltf)), e.forEachScene((r) => {
      this._addScene(r);
  ...
  _createMeshes(e) {
    ...
    let r;
    r = s ? t.binChunkBuffer : new X(this.gltf);
    ...
    const D = [];
    return e.components && e.components.length > 0 && e.components[0].faces ? (S = r.addBufferView(_.UNSIGNED_INT, g.SCALAR, y.ELEMENT_ARRAY_BUFFER), this._addMeshVertexIndexed(S, e.components, D, H, B, V, z, F)) : this._addMeshVertexNonIndexed(e.components, D, H, B, V, z, F), w.finalize(), d && d.finalize(), m && m.finalize(), A && A.finalize(), S && S.finalize(), T && T.finalize(), s || r.finalize(), D;
  }
  _addMaterial(e) {
    if (e === null) return;
...
class ze {
  constructor(e) {
    this.mesh = e, this.name = "", this.translation = M(), this.rotation = pe(), this.scale = v(Q), this._nodes = [];
  }
  addNode(e) {
    if (this._nodes.includes(e)) throw new Error("Node already added");
    this._nodes.push(e);
...
//# sourceMappingURL=gltfexport-CfjbjRgo.js.map
