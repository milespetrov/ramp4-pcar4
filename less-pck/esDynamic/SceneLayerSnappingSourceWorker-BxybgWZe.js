import { lw as Re, bQ as I, dh as Z, lj as Ae, aI as F, dk as y, dl as B, li as ee, aJ as p, kg as te, le as X, O as Ee, Q as Se, dm as Fe, dg as C } from "./main-n1d2W0Ts.js";
import { v as Ne, h as xe, M as Me } from "./lineSegment-FPBx0bds.js";
import { s as le, m as ne, T as v, l as oe, N as k, H as ie, V as _, _ as j, a as _e } from "./sphere-C-09q0LQ.js";
import "./plane-fUOqQtDL.js";
import { i as fe } from "./Util-D-9umDoA.js";
import { m as je } from "./edgeProcessing-myGljhiX.js";
function me(o) {
...
let D = class W {
  ...
  destroy() {
    this._degenerateObjects.clear(), u.clearPool(), Q[0] = null, x.prune(), M.prune();
  }
  add(e, t = e.length) {
  ...
  forEachInDepthRange(e, t, n, i, s, a, h) {
    ...
    f.setRange(i);
    const l = S(t, n, e), T = q(t, n), R = q(t, -n), P = (m) => {
      if (!h(m)) return;
  ...
  forEachNeighbor(e, t) {
    const n = k(t), i = _(t), s = (r) => {
      const d = this.objectToBoundingSphere(r), f = k(d), l = n + f;
  ...
  _rebuildTree(e, t) {
    Z(_(J), _(t.bounds)), J[3] = t.halfSize, ae([e, J], 2, (i) => i, K);
    const n = u.acquire().init(this._root);
    this._root.initFrom(null, K, K[3]), this._root.increaseHalfSize(1.25), N(n, (i) => (this.add(i.node.terminals.data, i.node.terminals.length), i.node.residents !== null && this.add(i.node.residents.data, i.node.residents.length), !0)), u.release(n);
  }
  ...
  _rootBoundsForRootAsSubNode(e) {
    ...
    for (let r = 0; r < 3; r++) {
      ...
      i = Math.max(i, R), w[r].min = l, w[r].max = T;
    }
    for (let r = 0; r < 3; r++) {
      let d = w[r].min, f = w[r].max;
      const l = (i - (d + f)) / 2;
    ...
    const h = i * a;
    return V[3] = h * Oe, u.acquire().initFrom(null, V, h, 0);
  }
...
function Ce(o, e, t) {
  if (!M.length) for (let n = 0; n < 8; ++n) M.push({ index: 0, distance: 0 });
  for (let n = 0; n < 8; ++n) {
    const i = Te[n];
    M.data[n].index = n, M.data[n].distance = S(o, e, i);
  }
  M.sort((n, i) => n.distance - i.distance);
  for (let n = 0; n < 8; ++n) t[n] = M.data[n].index;
}
function q(o, e) {
  let t, n = 1 / 0;
  for (let i = 0; i < 8; ++i) {
...
const Te = [p(-1, -1, -1), p(1, -1, -1), p(-1, 1, -1), p(1, 1, -1), p(-1, -1, 1), p(1, -1, 1), p(-1, 1, 1), p(1, 1, 1)], he = [p(-1, -1, -1), p(-1, -1, 1), p(-1, 1, -1), p(-1, 1, 1), p(1, -1, -1), p(1, -1, 1), p(1, 1, -1), p(1, 1, 1)], Oe = Math.sqrt(3), Q = [null];
function ve(o) {
  return Q[0] = o, Q;
}
const V = j(), g = F(), O = F(), b = F(), x = new I(), De = j(), A = j(), J = j(), K = j(), w = [{ min: 0, max: 0 }, { min: 0, max: 0 }, { min: 0, max: 0 }], M = new I(), de = [0, 0, 0, 0, 0, 0, 0, 0], ce = D, Ge = 1e3;
function $e(o, e, t) {
  const n = j(), i = _(n);
...
let U = class {
  ...
  _setFetchEdgeLocations(o) {
    ...
    const t = je.createView(o.locations), n = new Array(t.count), i = F(), s = F();
  ...
  _addEdgeCandidate(o, e, t, n, i) {
    ...
    const s = _(o.bounds), a = xe(t, n, this._tmpLineSegment), h = Me(a, s, this._tmpP3);
    _e(o.bounds, h) && i.push({ type: "edge", objectId: e, target: C(h), distance: X(s, h), start: C(t), end: C(n) });
  }
};
U = Ee([Se("esri.views.interactive.snapping.featureSources.sceneLayerSource.SceneLayerSnappingSourceWorker")], U);
const Qe = U;
function ue(o, e, t, n) {
  if (!o.returnVertex || !_e(o.bounds, t)) return;
  const i = _(o.bounds);
  n.push({ type: "vertex", objectId: e, target: C(t), distance: X(i, t) });
}
class $ {
  constructor(e, t) {
...
//# sourceMappingURL=SceneLayerSnappingSourceWorker-BxybgWZe.js.map
