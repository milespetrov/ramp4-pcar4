import { f3 as q, bv as T, cb as K } from "./main-n1d2W0Ts.js";
import { y as Q } from "./DoubleArray-DzeMZxkj.js";
import { i as W, a as X } from "./MeshLocalVertexSpace-h0SqnZHL.js";
import { f as Z, l as ee } from "./vec3-CQg9d0_-.js";
import "./sphere-C-09q0LQ.js";
import "./I3SUtil-CdyzHhWR.js";
var U, D;
(function(e) {
...
function te() {
  return R || (R = new Promise((e) => import("./i3s-DojQC6fw.js").then((t) => t.i).then(({ default: t }) => {
...
async function be(e) {
  const { localMatrix: t, origin: n, positions: f, vertexSpace: i, localMode: u } = e, c = T.fromJSON(e.inSpatialReference), l = T.fromJSON(e.outSpatialReference);
  let s;
  const [{ projectBuffer: I }, { initializeProjection: M }] = await Promise.all([import("./main-n1d2W0Ts.js").then((a) => a.s_), import("./main-n1d2W0Ts.js").then((a) => a.s$)]);
  await M(c, l);
  const g = [0, 0, 0];
  ...
  if (i.type === "georeferenced" && i.origin == null) {
    ...
    const a = i.type === "georeferenced" ? W.fromJSON(i) : X.fromJSON(i), { project: m } = await import("./projection-aThzGnG_.js"), d = m({ positions: f, transform: t ? { localMatrix: t } : void 0, vertexSpace: a, inSpatialReference: c, outSpatialReference: l, localMode: u });
...
function H(e, t, n) {
  ...
  const P = I, J = { isDraco: !1, isLegacy: !1, color: t.layouts.some((h) => h.some((y) => y.name === "color")), normal: t.needNormals && t.layouts.some((h) => h.some((y) => y.name === "normalCompressed")), uv0: t.layouts.some((h) => h.some((y) => y.name === "uv0")), uvRegion: t.layouts.some((h) => h.some((y) => y.name === "uvRegion")), featureIndex: P.featureIndex }, o = e.process(f, !!t.obbData, w, L.byteLength, P, J, E, l, M, g, t.normalReferenceFrame);
  if (e._free(E), e._free(w), o.error.length > 0) throw new Error(`i3s.wasm: ${o.error}`);
  if (o.discarded) return null;
  const F = o.componentOffsets.length > 0 ? o.componentOffsets.slice() : null, _ = o.featureIds.length > 0 ? o.featureIds.slice() : null, z = o.anchorIds.length > 0 ? Array.from(o.anchorIds) : null, V = o.anchors.length > 0 ? Array.from(o.anchors) : null, x = o.interleavedVertedData.slice().buffer, O = o.indicesType === U.Int16 ? new Uint16Array(o.indices.buffer, o.indices.byteOffset, o.indices.byteLength / 2).slice() : new Uint32Array(o.indices.buffer, o.indices.byteOffset, o.indices.byteLength / 4).slice(), v = o.positions.slice(), C = o.positionIndicesType === U.Int16 ? new Uint16Array(o.positionIndices.buffer, o.positionIndices.byteOffset, o.positionIndices.byteLength / 2).slice() : new Uint32Array(o.positionIndices.buffer, o.positionIndices.byteOffset, o.positionIndices.byteLength / 4).slice(), G = { layout: t.layouts[0], interleavedVertexData: x, indices: O, hasColors: o.hasColors, hasModifications: o.hasModifications, positionData: { data: v, indices: C } };
  return _ && n.push(_.buffer), F && n.push(F.buffer), n.push(x), n.push(O.buffer), n.push(v.buffer), n.push(C.buffer), { componentOffsets: F, featureIds: _, anchorIds: z, anchors: V, transformedGeometry: G, obb: o.obb, globalTrafo: i };
}
function we(e) {
...
//# sourceMappingURL=SceneLayerWorker-Dd5zKL0Z.js.map
