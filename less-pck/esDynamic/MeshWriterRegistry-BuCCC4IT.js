import { a as ae, c as ie, f as z, e as w } from "./UpdateTracking2D-BihgDBEV.js";
import { N as ee, m as ne, S as F, g as oe, i as ce, a as he, T as le, s as ue, v as de, _ as me, c as fe, h as pe, o as _e, b as ge, d as xe, e as ye } from "./PieChartMeshWriter-BIN5Wn3k.js";
import { lm as ve, c_ as N, ln as Me, lo as G, cN as O, bL as j, lp as X, fk as S, lq as Y, lr as V, ls as Pe, lt as H, lu as we, d5 as Z, fg as L, $ as B, ad as be, s as Ie, D as Se } from "./main-n1d2W0Ts.js";
import { r as E, i as Le, o as $e } from "./LabelMetric-DtXpnITF.js";
import { t as A } from "./constants-DVpDF9P6.js";
import { o as ke } from "./grouping-D-egGQUs.js";
import { s as ze } from "./AttributeStore-GRl8PvEZ.js";
function Q(c, e, r) {
...
class Ge extends ee {
  ...
  _writeAboveAndBelowAlong(e, r, t) {
    const { repeatLabel: s, fontSize: a } = this.evaluatedMeshParams, { shaping: i } = this._current, h = O(a), l = e.readGeometryForDisplay();
    if (!l) return;
    const d = new Z();
    ve(d, l, !1, !1, "esriGeometryPolyline", 1);
    const o = K(new Z(), d, h), n = K(new Z(), d, -h), u = N(n, "esriGeometryPolyline", !1, !1), p = N(o, "esriGeometryPolyline", !1, !1), _ = q(p.paths, i.bounds.width), m = q(u.paths, i.bounds.width);
    this._current.offsetDirection = "above";
    for (const f of _) W(f, t, r, !!s);
    this._current.offsetDirection = "below";
    for (const f of m) W(f, t, r, !!s);
  }
  _writeCenterAlong(e, r, t) {
    const { repeatLabel: s } = this.evaluatedMeshParams, { shaping: a } = this._current, i = q(e.readLegacyGeometryForDisplay().paths, a.bounds.width);
    for (const h of i) W(h, t, r, !!s);
  }
  ...
  _placeStraightAlong(e, r) {
    ...
    if (a.glyphs.length > 0 && !(!l && !d)) {
      const f = Math.max(r, i[0], 0), g = Math.min(F, i[1]), x = Me(G(), -e.angle), y = { minZoom: f, maxZoom: g, clipAngle: n, mapAligned: !0, isLineLabel: !0 }, I = O(this.evaluatedMeshParams.offsetX), P = O(this.evaluatedMeshParams.offsetY);
      if (!this._verticalPlacement || this._verticalPlacement === this._current.offsetDirection) {
        const v = j(I, -1 * P), [M, b] = a.shapeBackground(X(G(), x, v));
        t.recordStart(this.instanceId, this.attributeLayout, a.glyphs[0].textureBinding);
        const k = 2 * Math.max(M.width, M.height);
        t.recordBounds(e.x + M.x, e.y + M.y, k, k), this._writeTextBox(t, s, e.x, e.y, b, h, y), t.recordEnd();
      }
      if (!this._verticalPlacement || this._verticalPlacement !== this._current.offsetDirection) {
        const v = j(I, P), [M, b] = a.shapeBackground(X(G(), x, v));
  ...
  _placeBack(e, r, t, s, a, i) {
    const h = e.clone();
    let l = e.backwardLength + $;
    for (; h.prev() && !(l >= s); ) this._placeOnSegment(h, r, l, t, -1, a, i), l += h.length + $;
  }
  _placeForward(e, r, t, s, a, i) {
    const h = e.clone();
    let l = e.remainingLength + $;
    for (; h.next() && !(l >= s); ) this._placeOnSegment(h, r, l, t, 1, a, i), l += h.length + $;
  }
  _placeFirst(e, r, t, s, a = !1) {
    const { out: i, id: h, shaping: l, zoomRange: d, referenceBounds: o } = this._current, n = l.glyphs;
    for (const u of n) {
      const p = u.x > l.bounds.x ? t : 1 - t, _ = p * e.remainingLength + (1 - p) * e.backwardLength, m = Math.abs(u.x + u.width / 2 - l.bounds.x), f = Math.max(0, this._zoomLevel + Math.log2(m / (_ + $))), g = Math.max(r, a ? 0 : f);
  ...
  _placeOnSegment(e, r, t, s, a, i, h) {
    ...
    for (const f of u) {
      ...
      const x = Math.abs(f.x + f.width / 2 - o.bounds.x), y = Math.max(0, this._zoomLevel + Math.log2(x / t) - 0.1), I = Math.max(s, this._zoomLevel + Math.log2(x / (t + e.length + $)));
...
function K(c, e, r) {
  ...
  for (let p = 0; p < s.length; p++) {
    ...
    for (let m = 0; m < _; m++) {
      ...
      m > 0 ? S(a, t[f], t[f + 1]) : S(a, 0, 0), S(i, t[g], t[g + 1]), m < _ - 1 ? S(h, t[x], t[x + 1]) : S(h, 0, 0), m === 0 ? S(l, 0, 0) : (Y(l, i, a), V(l, l), S(l, l[1], -l[0])), m === _ - 1 ? S(d, 0, 0) : (Y(d, h, i), V(d, d), S(d, d[1], -d[0])), Pe(o, l, d), V(o, o);
...
const Ve = () => Se.getLogger("esri.views.2d.engine.webgl.shaderGraph.techniques.mesh.MeshWriterInputEvaluator");
async function R(c, e, r = !1) {
  ...
  return { ...e, computed: new Oe(a), defaultValue: t };
}
...
const re = (c) => typeof c != "boolean" && typeof c != "number" && "valueExpressionInfo" in c, Ze = (c) => c.some((e) => {
  for (const r in e) {
    const t = e[r];
    if (re(t)) return !0;
  }
...
class C {
  ...
  enqueueRequest(e, r, t) {
    for (const s of this._dynamicAsyncProperties.values()) {
      ...
      s.rasterizationParam.resource.type === "animated" && s.rasterizationParam.resource.randomizeStartTime && (a.primitiveName = "__RESERVED__PRIMITIVE__NAME__", a.startGroup = ke(r.getObjectId() || 0));
      for (const { primitiveName: h, propertyName: l, computed: d, defaultValue: o, valueExpressionInfo: n } of s.compiledOverrides) try {
        ...
        Ve().errorOnce(new Ie("invalid-arcade-expression", `Encountered an error when evaluating the arcade expression '${n?.expression}' (primitive: '${h}', property: '${l}')`, u));
      }
...
class He {
  async createMeshWriter(e, r, t, s) {
    const a = this._getMeshWriter(s.techniqueType), i = await C.create(e, r, s.inputParams), h = new a(s.id, i, s.optionalAttributes, t);
...
//# sourceMappingURL=MeshWriterRegistry-BuCCC4IT.js.map
