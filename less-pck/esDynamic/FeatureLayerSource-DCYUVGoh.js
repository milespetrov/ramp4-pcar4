import { K as U, eK as k, bp as j, eL as Q, eM as T, eN as P, D as v, s as q, _ as G, ey as V, eO as C, O as E, P as F, eP as H, eG as z, S as W, ac as M, eQ as X, eR as A, a2 as g, $ as x, eS as Y, eT as Z, eU as B, eV as K, eW as ee, b8 as te, eX as ae, eY as se, b3 as $, a4 as N, eZ as re, e_ as ne, er as ie, a1 as oe, ak as ue, bv as le, e$ as de, f0 as ce, f1 as pe, Q as he } from "./main-n1d2W0Ts.js";
import { i as ye } from "./MeshLocalVertexSpace-h0SqnZHL.js";
import { a as me } from "./meshVertexSpaceUtils-BSZfC57T.js";
import { N as ge, v as fe } from "./MeshTransform-mbyl_DYz.js";
import { isFeatureIdentifierArrayWithGlobalId as we, isFeatureIdentifierArrayWithObjectId as be } from "./editingSupport-DOC8Eybw.js";
import { o as Se } from "./clientSideDefaults-D1T7exSR.js";
async function J(t, e, a) {
  const { geometry: r } = e, s = { ...e.attributes };
  if (a != null && r?.type === "mesh") {
    const { transformFieldRoles: n } = a, { origin: i, spatialReference: o, vertexSpace: u } = r, d = r.transform ?? new ge(), l = u.type === "local", m = t.spatialReference, S = m.isGeographic, I = U(m, o), p = k(o, m) && j(o, m);
    if (!(l && S && p || !l && !S && I)) return null;
    const c = Q(i, o, m);
    if (c == null) return null;
    if (s[n.originX] = c.x, s[n.originY] = c.y, s[n.originZ] = c.z ?? 0, d != null) {
      const { translation: R, scale: h, rotation: f } = d, y = l ? 1 : T(o) / T(m);
      s[n.translationX] = R[0] * y, s[n.translationY] = R[2] * y, s[n.translationZ] = -R[1] * y, s[n.scaleX] = h[0], s[n.scaleY] = h[2], s[n.scaleZ] = h[1], s[n.rotationX] = f[0], s[n.rotationY] = f[2], s[n.rotationZ] = -f[1], s[n.rotationDeg] = f[3];
    }
    return { attributes: s };
  }
...
function qe(t) {
  const e = t?.assetMaps;
  if (e) {
    for (const s of e.addResults) s.success || v.getLogger("esri.layers.graphics.sources.support.sourceUtils").error(`Failed to map asset to feature with globalId ${s.globalId}.`);
    for (const s of e.updateResults) s.success || v.getLogger("esri.layers.graphics.sources.support.sourceUtils").error(`Failed to map asset to feature with globalId ${s.globalId}.`);
  }
...
function b(t) {
  ...
  return { objectId: t.objectId, globalId: t.globalId, error: e ? new q("feature-layer-source:edit-failure", e.description, { code: e.code }) : null };
}
function _(t, e) {
...
let O = class extends H {
  ...
  async applyEdits(t, e) {
    ...
    const { layer: a } = this;
    await A(a, "editing");
    const r = "infoFor3D" in a ? a.infoFor3D : null, s = r != null, n = s || (e?.globalIdUsed ?? !1), i = s ? await this._uploadMeshesAndGetAssetMapEditsJSON(t) : null, o = t.addFeatures?.map((w) => J(this.layer, w, r)) ?? [], u = (await Promise.all(o)).filter(x), d = t.updateFeatures?.map((w) => J(this.layer, w, r)) ?? [], l = (await Promise.all(d)).filter(x), m = Re(this.layer, t.deleteFeatures, n);
    Y(u, l, a.spatialReference);
    const S = await Ie(this.layer, t), I = a.capabilities.editing.supportsAsyncApplyEdits && s, p = e?.gdbVersion || a.gdbVersion, c = { gdbVersion: p, rollbackOnFailure: e?.rollbackOnFailureEnabled, useGlobalIds: n, returnEditMoment: e?.returnEditMoment, usePreviousEditMoment: e?.usePreviousEditMoment, async: I };
    await Z(this.layer.url, p, !0);
    const R = B(this.layer.url, p || null);
    if (await K(a.url, p, a.historicMoment)) throw new q("feature-layer-source:historic-version", "Editing a historic version is not allowed");
    e?.returnServiceEditsOption ? (c.edits = JSON.stringify([{ id: a.layerId, adds: u.length ? u : null, updates: l.length ? l : null, deletes: m.length ? m : null, attachments: S, assetMaps: i }]), c.returnServiceEditsOption = Fe.toJSON(e?.returnServiceEditsOption), c.returnServiceEditsInSourceSR = e?.returnServiceEditsInSourceSR) : (c.adds = u.length ? JSON.stringify(u) : null, c.updates = l.length ? JSON.stringify(l) : null, c.deletes = m.length ? n ? JSON.stringify(m) : m.join(",") : null, c.attachments = S && JSON.stringify(S), c.assetMaps = i != null ? JSON.stringify(i) : void 0);
    ...
    const f = e?.returnServiceEditsOption ? a.url : a.parsedUrl.path;
    let y;
    try {
      y = I ? await this._asyncApplyEdits(f + "/applyEdits", h) : await g(f + "/applyEdits", h);
    } catch (w) {
      if (!Ee(w)) throw w;
      h.authMode = "immediate", y = I ? await this._asyncApplyEdits(f + "/applyEdits", h) : await g(f + "/applyEdits", h);
    }
  ...
  async deleteAttachments(t, e) {
    ...
    try {
      return (await g(s, this._getLayerRequestOptions({ query: { attachmentIds: e.join(",") }, method: "post" }))).data.deleteAttachmentResults.map(b);
    } catch (n) {
  ...
  fetchRecomputedExtents(t = {}) {
    ...
    return this.load({ signal: e }).then(async () => {
      const a = this._getLayerRequestOptions({ ...t, query: { returnUpdates: !0 } }), { layerId: r, url: s } = this.layer, { data: n } = await g(`${s}/${r}`, a), { id: i, extent: o, fullExtent: u, timeExtent: d } = n, l = o || u;
  ...
  async fetchPublishingStatus() {
    ...
    const t = $(this.layer.url, "status"), e = await g(t, { query: { f: "json" } });
    return Ae.fromJSON(e.data.status);
  }
  async uploadAssets(t, e) {
    const { uploadAssets: a } = await import("./uploadAssets-4ZqxII2x.js");
  ...
  async _asyncApplyEdits(t, e) {
    ...
    for (; ; ) {
      const r = (await g(a, { query: { f: "json" }, responseType: "json" })).data;
      switch (r.status) {
        case "Completed":
          return g(r.resultUrl, { query: { f: "json" }, responseType: "json" });
        case "CompletedWithErrors":
          throw new q("async-applyEdits-failed", "asynchronous applyEdits call failed.");
        ...
        case "UnRegisteringReplica":
          break;
        default:
          throw new q("async-applyEdits-failed", "asynchronous applyEdits call failed (undefined response status)");
      }
      await ne(_e);
    }
  }
  ...
  async _fetchService(t, e) {
    if (!t) {
      ...
      const { data: s } = await g(this.layer.parsedUrl.path, this._getLayerRequestOptions({ query: r, signal: e?.signal }));
      t = s;
    }
    this.sourceJSON = await this._patchServiceJSON(t, e?.signal);
    const a = t.type;
    if (!this.supportedSourceTypes.has(a)) throw new q("feature-layer-source:unsupported-type", `Source type "${a}" is not supported`);
  }
  async _patchServiceJSON(t, e) {
    if (t.type !== "Table" && t.geometryType && !t?.drawingInfo?.renderer && !t.defaultSymbol) {
      const a = Se(t.geometryType).renderer;
      ie("drawingInfo.renderer", a, t);
    }
    if (t.geometryType === "esriGeometryMultiPatch" && t.infoFor3D && (t.geometryType = "mesh"), t.extent == null) try {
      const { data: a } = await g(this.layer.url, this._getLayerRequestOptions({ signal: e }));
  ...
  async _uploadMeshesAndGetAssetMapEditsJSON(t) {
    ...
    if (e.length !== a.length + t.updateFeatures.length) throw new q("feature-layer-source:unsupported-mesh-edits", "Mixing attribute only edits with mesh geometry edits is not currently supported");
    const r = new Array(), s = /* @__PURE__ */ new Map();
    for (const n of e) {
      const { geometry: i } = n, { vertexSpace: o } = i;
      if (me(o)) r.push(i);
      else {
        const u = i.anchor, { convertMeshVertexSpace: d } = await import("./convertMeshVertexSpace-U_UxnxGw.js"), l = await d(i, new ye({ origin: [u.x, u.y, u.z ?? 0] }));
  ...
  _getAssetMapEditsJSON(t) {
    ...
    for (const s of t) {
      ...
      if (o.length === 0) {
        v.getLogger(this).error(`Skipping feature ${u}. The mesh it is associated with has not been uploaded to the service and cannot be mapped to it.`);
        continue;
      }
      const { source: d } = o.find(fe) ?? o[0];
      for (const l of d) l.parts.length === 1 ? e.push({ globalId: ue(), parentGlobalId: u, assetName: l.assetName, assetHash: l.parts[0].partHash, flags: [] }) : v.getLogger(this).error(`Skipping asset ${l.assetName}. It does not have exactly one part, so we cannot map it to a feature.`);
    }
    return e;
  }
  ...
  _createAttachmentErrorResult(t, e) {
    ...
    return { objectId: t, globalId: null, error: new q("feature-layer-source:attachment-failure", a, { code: r }) };
  }
  ...
  async _areAllAssetsAlreadyMapped(t) {
    ...
    const u = o.map((p) => p.getAttribute(a)).filter(x);
    if (u.length === 0) return !1;
    const { assetMapFieldRoles: { parentGlobalId: d, assetHash: l } } = s, m = new ce({ where: `${d} IN (${u.map((p) => `'${p}'`)})`, outFields: [l, d], returnGeometry: !1 }), S = await pe(i, m), { features: I } = S;
    return I.length !== 0 && !o.some((p) => {
      const c = p.getAttribute(a);
      if (!c) return !0;
      const { metadata: R } = p.geometry, h = I.filter((y) => y.getAttribute(d) === c);
      if (h.length === 0) return !0;
      const f = h.map((y) => y.getAttribute(l));
      return R.getExternalSourcesOnService(r).flatMap(({ source: y }) => y.flatMap((w) => w.parts.map((L) => L.partHash))).some((y) => f.every((w) => y !== w));
    });
  }
};
...
//# sourceMappingURL=FeatureLayerSource-DCYUVGoh.js.map
