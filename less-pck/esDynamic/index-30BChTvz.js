import { mp as O, l2 as I, mq as d, hk as x, mr as P, l6 as R, ms as k, l4 as _, mt as T, bW as z, fw as F, fx as H, fO as N, fP as f, fA as v, hH as E, fU as W, f_ as m, fR as q, fV as D, fC as j, fI as y, fH as c, fK as K, fD as U, fM as S, hq as L, fE as g, kK as X } from "./main-n1d2W0Ts.js";
class Y extends O {
  ...
  async addMapGraphicLayer() {
    ...
    const e = new R(this.$iApi.geo.map.getExtent(), "overview-graphic"), a = this.overviewmapStore.borderColour ?? "#FF0000", i = this.overviewmapStore.borderWidth ?? 1, t = this.overviewmapStore.areaColour ?? "#000000", s = this.overviewmapStore.areaOpacity ?? 0.25, n = `${t}${Math.round(s * 255).toString(16)}`;
    e.style = new k({
      fill: { colour: n },
      outline: {
        colour: a,
  ...
  findBasemap(e) {
    ...
    if (a)
      return a;
    {
      const t = _(this.$vApp.$pinia).config.map;
      if (t) {
...
const J = { class: "relative h-full w-full overflow-hidden" }, Q = { class: "absolute h-30 w-30 top-0 right-0" }, ee = ["content", "aria-label"], te = /* @__PURE__ */ F({
  __name: "overviewmap",
  setup(u) {
    const e = d(), { t: a } = H(), i = N("iApi"), t = _(), s = f(), n = v(() => t.activeBasemapConfig), G = v(() => e.mapConfig), b = v(() => e.basemaps), A = v(() => e.startMinimized);
    let r = E(new Y(i));
    const p = f(!0), M = f(!1), l = E([]);
    W(() => {
      i.geo.map.viewPromise.then(async () => {
        w(), r.createMap(G.value, s.value.querySelector(".overviewmap")), await r.viewPromise, await r.addMapGraphicLayer(), p.value = A.value;
...
//# sourceMappingURL=index-30BChTvz.js.map
