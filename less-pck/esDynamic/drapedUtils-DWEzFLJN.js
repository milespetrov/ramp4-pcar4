import { cy as ee, cz as te, mj as re, mk as se, ml as G, eJ as q, O as a, P as p, mm as ie, ey as ne, ek as B, b8 as M, bv as oe, eX as ae, Q as T, bV as D, c2 as le, _, dO as ue, jb as pe, b$ as ye, al as ce, c1 as he, c0 as fe, a2 as de, dH as me, eG as ge, bi as be, s as U, J as E, V as we, $ as H, dV as z, c4 as xe, bo as P, ac as ve, mn as $e, dm as J, mo as Se, cM as Fe } from "./main-n1d2W0Ts.js";
import { n as Re, p as Oe } from "./popupUtils-DssuW32c.js";
function V(t, e) {
...
function Q(t, e) {
  return typeof t == "number" ? t : t?.stops?.length ? je(t.stops) : e;
}
function Me(t, e) {
  if (!e) return t;
  const s = e.filter((n) => n.type === "size").map((n) => {
    const { maxSize: c, minSize: u } = n;
    return (Q(c, t) + Q(u, t)) / 2;
  });
  let i = 0;
  const r = s.length;
  if (r === 0) return t;
...
function k(t) {
  ...
  if (!e) return i;
  const r = "visualVariables" in e ? Me(i, e.visualVariables) : i;
...
function Ee(t, e) {
  const { dpi: s, gdbVersion: i, geometry: r, geometryPrecision: o, height: n, historicMoment: c, layerOption: u, mapExtent: l, maxAllowableOffset: h, returnFieldName: y, returnGeometry: m, returnUnformattedValues: v, returnZ: d, spatialReference: S, timeExtent: b, tolerance: g, width: w } = t.toJSON(), { dynamicLayers: R, layerDefs: O, layerIds: A } = Ve(t), W = e?.geometry != null ? e.geometry : null, x = { historicMoment: c, geometryPrecision: o, maxAllowableOffset: h, returnFieldName: y, returnGeometry: m, returnUnformattedValues: v, returnZ: d, tolerance: g }, j = W?.toJSON() || r;
  x.imageDisplay = `${w},${n},${s}`, i && (x.gdbVersion = i), j && (delete j.spatialReference, x.geometry = JSON.stringify(j), x.geometryType = ee(j));
  const L = S ?? j?.spatialReference ?? l?.spatialReference;
  if (L && (x.sr = te(L)), x.time = b ? [b.start, b.end].join(",") : null, l) {
    const { xmin: C, ymin: X, xmax: K, ymax: Y } = l;
    x.mapExtent = `${C},${X},${K},${Y}`;
  }
  return O && (x.layerDefs = O), R && !O && (x.dynamicLayers = R), x.layers = u === "popup" ? "visible" : u, A && !R && (x.layers += `:${A.join(",")}`), x;
}
function Ve(t) {
...
async function Ne(t, e, s) {
  ...
  return r.path += "/identify", ce(i).then((o) => {
    const n = Ee(e, { geometry: o?.[0] }), c = he({ ...r.query, f: "json", ...n }), u = fe(c, s);
...
let F = class extends me {
  ...
  highlight(t) {
    ...
    if (!e) return E();
    let s = null;
    if (t instanceof _ ? s = [t] : we.isCollection(t) && t.length > 0 ? s = t.toArray() : Array.isArray(t) && t.length > 0 && (s = t), s = s?.filter(H), !s?.length) return E();
    for (const i of s) {
      const r = i.sourceLayer;
      r != null && "geometryType" in r && r.geometryType === "point" && (i.visible = !1);
    }
    return e.addMany(s), E(() => e.removeMany(s ?? []));
  }
  ...
  async _createIdentifyParameters(t, e, s) {
    ...
    const u = Math.min(ve("mapservice-popup-identify-max-tolerance"), r.allSublayers.reduce((v, d) => d.renderer ? k({ renderer: d.renderer, pointerType: s?.pointerType }) : v, 2)), l = this.createFetchPopupFeaturesQueryGeometry(t, u), h = $e(c, n), y = Math.round(l.width / h), m = new M({ xmin: l.center.x - h * y, ymin: l.center.y - h * y, xmax: l.center.x + h * y, ymax: l.center.y + h * y, spatialReference: l.spatialReference });
  ...
  async _fetchPopupFeaturesUsingQueries(t, e, s) {
    const { layerView: { floors: i, timeExtent: r } } = this, o = e.map(async ({ sublayer: n, popupTemplate: c }) => {
      ...
      const u = n.createQuery(), l = k({ renderer: n.renderer, pointerType: s?.pointerType }), h = this.createFetchPopupFeaturesQueryGeometry(t, l), y = /* @__PURE__ */ new Set(), [m] = await Promise.all([Re(n, c), n.renderer?.collectRequiredFields(y, n.fieldsIndex)]);
      J(s), Se(y, n.fieldsIndex, m);
      ...
      const R = g ? 0 : S;
      w = await He(n, w, s);
      for (const O of w) this._featuresResolutions.set(O, R);
      return w;
    });
...
//# sourceMappingURL=drapedUtils-DWEzFLJN.js.map
