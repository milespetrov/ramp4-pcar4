import { O as t, P as r, Q as R, eP as _, eG as U, U as E, b8 as I, s as k, dw as f, dE as J, ez as W, ac as X, D as Y, bg as D, iL as L, dZ as Q, iM as H, iN as G, dI as V, iO as Z, dJ as z, dM as M, dK as A, dL as B, dN as K, bv as T, iP as O, iQ as ee, dv as C, er as te, I as re, iR as ie, f0 as c, iS as se, e4 as oe, ek as ae, iT as ne, iU as le, iV as pe, dQ as de, jg as h, jL as ue, iX as ye, W as ce, iY as me, iZ as he, i_ as fe, dR as ge, j0 as we, dX as ve } from "./main-n1d2W0Ts.js";
import { l as Fe, o as xe } from "./clientSideDefaults-D1T7exSR.js";
import { v as be, W as Oe, z as Se, Y as Re, S as Ie } from "./wfsUtils-iN6tLKv3.js";
let d = class extends _ {
  ...
  async _createLoadOptions(e) {
    ...
    const P = ["fields", "geometryType", "name", "namespaceUri", "swapXY"].some((F) => this.layer[F] == null), p = P ? await Oe(this.wfsCapabilities, n, l, { spatialReference: m, customParameters: o, signal: e?.signal }) : { ...Se(a ?? []), geometryType: u, name: n, namespaceUri: l, spatialReference: m, swapXY: v }, j = Re(this.wfsCapabilities.readFeatureTypes(), p.name, p.namespaceUri), $ = f.toJSON(p.geometryType), { operations: b } = this.wfsCapabilities, q = b.GetFeature.url, N = b.GetFeature.outputFormat;
    return { customParameters: o, featureType: j, fields: p.fields?.map((F) => F.toJSON()) ?? [], geometryField: p.geometryField, geometryType: $, getFeatureUrl: q, getFeatureOutputFormat: N, maxRecordCount: y, maxPageCount: g, maxTotalRecordCount: w, objectIdField: p.objectIdField, spatialReference: p.spatialReference?.toJSON(), swapXY: !!p.swapXY };
  }
  async _startWorker(e) {
    const [s, o] = await J([this._createLoadOptions(e), W("WFSSourceWorker", { ...e, strategy: X("feature-layers-workers") ? "dedicated" : "local", registryTarget: this })]), n = s.error || o.error || null, l = o.value || null;
...
var x;
const S = we();
let i = x = class extends L(Q(H(G(V(Z(z(M(A(B(K(ve))))))))))) {
  ...
  writeFields(e, s, o) {
    ...
    this.geometryField && n.unshift(new C({ name: this.geometryField, alias: this.geometryField, type: "geometry" })), te(o, n.map((l) => l.toJSON()), s);
  }
  get parsedUrl() {
    return re(this.url);
  }
