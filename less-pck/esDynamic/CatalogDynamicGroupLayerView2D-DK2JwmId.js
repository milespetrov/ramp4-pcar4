import { O as a, P as i, Q as f, V, eG as F, eH as b, J as U, eI as H, D as I, eJ as w } from "./main-n1d2W0Ts.js";
import { f as _, y as x } from "./LayerView-C5VGrkrZ.js";
const u = Symbol(), O = (o) => {
  let e = class extends o {
    ...
    updateLayers() {
      this.suspended ? this.removeHandles(u) : this._updatingHandles.addPromise(H(this._debouncedUpdate()).catch((r) => {
        I.getLogger(this).error(r);
      }));
    }
    _createQuery() {
      ...
      if (!n) return null;
      const { itemTypeField: l, itemSourceField: h, itemNameField: c, minScaleField: m, maxScaleField: L, objectIdField: v, orderBy: $ } = s, S = w(`${m} IS NULL OR ${d} <= ${m} OR ${m} = 0`, `${L} IS NULL OR ${d} >= ${L}`), p = $?.find((y) => y.field && !y.valueExpression), t = r.createQuery();
...
//# sourceMappingURL=CatalogDynamicGroupLayerView2D-DK2JwmId.js.map
