import { e as Ot } from "./deduplicate-Bl0flx-L.js";
import { H as F } from "./InterleavedLayout-GBDER6W8.js";
import { e as l } from "./VertexAttribute-CAkzp1tV.js";
import { C as V } from "./enums-DDkmfb-t.js";
import { t as St } from "./VertexElementDescriptor-BAy1DPb3.js";
import { aI as T, bQ as tt, le as Et, fo as k, dh as rt, aE as K, lc as At, mY as Tt, aF as dt, di as at, aH as ht, ge as Nt, aP as wt, dl as $t, mZ as yt } from "./main-n1d2W0Ts.js";
function et(t, e = 0) {
  const o = t.stride;
...
const H = -1;
function xt(t, e, o) {
  const s = t.vertices.position, r = t.vertices.componentIndex, c = O.position0, d = O.position1, h = O.faceNormal0, g = O.faceNormal1, { edges: i, normals: p } = Ct(t), m = i.length / 4, E = e.allocate(m);
  let P = 0;
  ...
  for (let N = 0; N < m; ++N) {
    ...
    const D = z.pushNew();
    D.index = 4 * N, D.length = Et(c, d);
  }
  z.sort((N, y) => y.length - N.length);
  const f = new Array(), u = new Array();
  z.forAll(({ length: N, index: y }) => {
    const D = i.data[y], It = i.data[y + 1], nt = i.data[y + 2], ot = i.data[y + 3], st = ot === H;
    if (s.getVec(D, c), s.getVec(It, d), st) {
      const A = 3 * nt;
      k(h, p.data[A], p.data[A + 1], p.data[A + 2]), rt(g, h), O.componentIndex = r.get(D), O.cosAngle = K(h, g);
    } else {
      let A = 3 * nt;
      if (k(h, p.data[A], p.data[A + 1], p.data[A + 2]), A = 3 * ot, k(g, p.data[A], p.data[A + 1], p.data[A + 2]), O.componentIndex = r.get(D), O.cosAngle = K(h, g), Lt(O, _t)) return;
      O.cosAngle < -0.9999 && rt(g, h);
    }
    n += N, a++, st || Dt(O, Ut) ? (e.write(E, P++, O), f.push(N)) : bt(O, gt) && (w && o && o.write(w, _++, O), u.push(N));
  });
  const S = new Float32Array(f.reverse()), $ = new Float32Array(u.reverse()), M = w && o ? { instancesData: w.slice(0, _), lodInfo: { lengths: $ } } : void 0;
  return { regular: { instancesData: E.slice(0, P), lodInfo: { lengths: S } }, silhouette: M, averageEdgeLength: n / a };
}
function Dt(t, e) {
  return t.cosAngle < e;
}
...
function bt(t, e) {
  ...
  return Tt(it, t.position1, t.position0), o * (K(dt(Bt, t.faceNormal0, t.faceNormal1), it) > 0 ? -1 : 1) > e;
}
function Ct(t) {
  ...
  for (let c = 0; c < e; c++) {
    ...
    r.getVec(p, x), r.getVec(m, U), r.getVec(E, X), at(U, U, x), at(X, X, x), dt(x, U, X), ht(x, x), G.pushArray(x), (h === H || p < m) && (I.push(p), I.push(m), I.push(c), I.push(h)), (g === H || m < E) && (I.push(m), I.push(E), I.push(c), I.push(g)), (i === H || E < p) && (I.push(E), I.push(p), I.push(c), I.push(i));
  }
  return { edges: I, normals: G };
}
class Ft {
  constructor() {
...
const z = new tt({ allocator: (t) => t || new Ft(), deallocator: null }), I = new tt({ deallocator: null }), G = new tt({ deallocator: null }), O = new Vt(), Bt = T(), it = T(), x = T(), U = T(), X = T(), gt = Nt(4), _t = Math.cos(gt), zt = Nt(35), Ut = Math.cos(zt);
function ct(t, e, o) {
...
let mt = class {
  updateSettings(e) {
    this.settings = e, this._edgeHashFunction = e.reducedPrecision ? Wt : Xt;
  }
  write(e, o, s) {
    W.seed = this._edgeHashFunction(s);
    const r = W.getIntRange(0, 255), c = W.getIntRange(0, this.settings.variants - 1), d = W.getFloat(), h = 255 * (0.5 * Ht(-(1 - Math.min(d / q, 1)) + Math.max(0, d - q) / (1 - q), 1.2) + 0.5);
...
const v = new Float32Array(6), R = new Uint32Array(v.buffer), C = new Uint32Array(1);
function Xt(t) {
  return v[0] = t.position0[0], v[1] = t.position0[1], v[2] = t.position0[2], v[3] = t.position1[0], v[4] = t.position1[1], v[5] = t.position1[2], C[0] = 31 * (31 * (31 * (31 * (31 * (166811 + R[0]) + R[1]) + R[2]) + R[3]) + R[4]) + R[5], C[0];
}
function Wt(t) {
  const e = v;
  e[0] = L(t.position0[0]), e[1] = L(t.position0[1]), e[2] = L(t.position0[2]), e[3] = L(t.position1[0]), e[4] = L(t.position1[1]), e[5] = L(t.position1[2]), C[0] = 5381;
  for (let o = 0; o < R.length; o++) C[0] = 31 * C[0] + R[o];
  return C[0];
}
const ut = 1e4;
function L(t) {
...
class Q {
  ...
  write(e, o, s) {
    this._commonWriter.write(e, o, s), $t(b, s.faceNormal0, s.faceNormal1), ht(b, b);
    const { typedBuffer: r, typedBufferStride: c } = e.normalCompressed;
    J(r, o, b[0], b[1], b[2], c);
  }
}
Q.Layout = Y, Q.glLayout = et(Y, 1);
class Z {
  constructor() {
    this._commonWriter = new mt();
  }
...
Z.Layout = j, Z.glLayout = et(j, 1);
const b = T(), W = new yt();
function ee(t) {
  const e = kt(t.data, t.skipDeduplicate, t.indices, t.indicesLength);
  return ft.updateSettings(t.writerSettings), pt.updateSettings(t.writerSettings), xt(e, ft, pt);
}
function kt(t, e, o, s) {
  if (e) {
    const d = ct(o, s, t.count);
    return new Gt(o, s, d, t);
  }
  const r = Ot(t.buffer, t.stride / 4, { originalIndices: o, originalIndicesLength: s }), c = ct(r.indices, s, r.uniqueCount);
...
//# sourceMappingURL=edgeProcessing-myGljhiX.js.map
