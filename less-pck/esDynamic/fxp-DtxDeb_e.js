...
var I = {}, P = {};
(function(e) {
  const t = ":A-Za-z_\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD", s = t + "\\-.\\d\\u00B7\\u0300-\\u036F\\u203F-\\u2040", i = "[" + t + "][" + s + "]*", n = new RegExp("^" + i + "$"), r = function(u, o) {
    const l = [];
    let a = o.exec(u);
    for (; a; ) {
      const d = [];
      d.startIndex = o.lastIndex - a[0].length;
      const c = a.length;
      for (let p = 0; p < c; p++)
        d.push(a[p]);
      l.push(d), a = o.exec(u);
  ...
  }, e.merge = function(u, o, l) {
    if (o) {
      const a = Object.keys(o), d = a.length;
      for (let c = 0; c < d; c++)
        l === "strict" ? u[a[c]] = [o[a[c]]] : u[a[c]] = o[a[c]];
    }
  }, e.getValue = function(u) {
    return e.isExist(u) ? u : "";
  }, e.isName = f, e.getAllMatches = r, e.nameRegexp = i;
})(P);
const C = P, G = {
...
` && e[r] !== "\r"; r++)
          o += e[r];
        if (o = o.trim(), o[o.length - 1] === "/" && (o = o.substring(0, o.length - 1), r--), !K(o)) {
          let d;
          return o.trim().length === 0 ? d = "Invalid space after '<'." : d = "Tag '" + o + "' is an invalid name.", g("InvalidTag", d, N(e, r));
        }
        const l = Y(e, r);
        if (l === !1)
        ...
        if (r = l.index, a[a.length - 1] === "/") {
          ...
          if (c === !0)
            ...
            return g(c.err.code, c.err.msg, N(e, d + c.err.line));
        } else if (u)
          if (l.tagClosed) {
            if (a.trim().length > 0)
              return g("InvalidTag", "Closing tag '" + o + "' can't have attributes or invalid starting.", N(e, f));
            {
              const d = s.pop();
              if (o !== d.tagName) {
                let c = N(e, d.tagStartPos);
                return g(
                  "InvalidTag",
                  "Expected closing tag '" + d.tagName + "' (opened in line " + c.line + ", col " + c.col + ") instead of closing tag '" + o + "'.",
                  N(e, f)
                );
              }
              s.length == 0 && (n = !0);
          ...
          const d = _(a, t);
          if (d !== !0)
            return g(d.err.code, d.err.msg, N(e, r - a.length + d.err.line));
          if (n === !0)
            return g("InvalidXml", "Multiple possible root nodes found.", N(e, r));
          t.unpairedTags.indexOf(o) !== -1 || s.push({ tagName: o, tagStartPos: f }), i = !0;
        }
        for (r++; r < e.length; r++)
          if (e[r] === "<")
            if (e[r + 1] === "!") {
  ...
  if (i) {
    if (s.length == 1)
      return g("InvalidTag", "Unclosed tag '" + s[0].tagName + "'.", N(e, s[0].tagStartPos));
    if (s.length > 0)
      return g("InvalidXml", "Invalid '" + JSON.stringify(s.map((r) => r.tagName), null, 4).replace(/\r?\n/g, "") + "' found.", { line: 1, col: 1 });
...
function _(e, t) {
  const s = C.getAllMatches(e, J), i = {};
  for (let n = 0; n < s.length; n++) {
    if (s[n][1].length === 0)
      return g("InvalidAttr", "Attribute '" + s[n][2] + "' has no space in starting.", m(s[n]));
    if (s[n][3] !== void 0 && s[n][4] === void 0)
      return g("InvalidAttr", "Attribute '" + s[n][2] + "' is without value.", m(s[n]));
    if (s[n][3] === void 0 && !t.allowBooleanAttributes)
      return g("InvalidAttr", "boolean attribute '" + s[n][2] + "' is not allowed.", m(s[n]));
    const r = s[n][2];
    if (!z(r))
      return g("InvalidAttr", "Attribute '" + r + "' is an invalid name.", m(s[n]));
    if (!i.hasOwnProperty(r))
      i[r] = 1;
    else
      return g("InvalidAttr", "Attribute '" + r + "' is repeated.", m(s[n]));
  }
  return !0;
}
function q(e, t) {
...
function ee(e, t) {
  ...
  if (e[t + 3] === "O" && e[t + 4] === "C" && e[t + 5] === "T" && e[t + 6] === "Y" && e[t + 7] === "P" && e[t + 8] === "E") {
    ...
    for (; t < e.length; t++)
      if (e[t] === "<" && !r) {
        if (n && ne(e, t))
          t += 7, [entityName, val, t] = te(e, t + 1), val.indexOf("&") === -1 && (s[ue(entityName)] = {
            regx: RegExp(`&${entityName};`, "g"),
            val
          });
        else if (n && re(e, t)) t += 8;
        else if (n && ie(e, t)) t += 8;
        else if (n && oe(e, t)) t += 9;
        else if (se) r = !0;
...
var fe = ee;
const le = /^[-+]?0x[a-fA-F0-9]+$/, ae = /^([\-\+])?(0*)(\.[0-9]+([eE]\-?[0-9]+)?|[0-9]+(\.[0-9]+([eE]\-?[0-9]+)?)?)$/;
...
var ge = ce;
const $ = P, A = H, pe = fe, Ne = ge;
"<((!\\[CDATA\\[([\\s\\S]*?)(]]>))|((NAME:)?(NAME))([^>]*)>|((\\/)(NAME)\\s*>))([^<]*)".replace(/NAME/g, $.nameRegexp);
let be = class {
  constructor(t) {
    ...
    }, this.addExternalEntities = Ee, this.parseXml = Pe, this.parseTextData = Te, this.resolveNameSpace = ye, this.buildAttributesMap = Ae, this.isItStopNode = Ce, this.replaceEntitiesValue = we, this.readStopNodeData = $e, this.saveTextToParentTag = Ie, this.addChild = Oe;
  }
};
function Ee(e) {
...
function Te(e, t, s, i, n, r, f) {
  if (e !== void 0 && (this.options.trimValues && !i && (e = e.trim()), e.length > 0)) {
    f || (e = this.replaceEntitiesValue(e));
    const u = this.options.tagValueProcessor(t, e, s, n, r);
    return u == null ? e : typeof u != typeof e || u !== e ? u : this.options.trimValues ? w(e, this.options.parseTagValue, this.options.numberParseOptions) : e.trim() === e ? w(e, this.options.parseTagValue, this.options.numberParseOptions) : e;
  }
}
function ye(e) {
  if (this.options.removeNSPrefix) {
    const t = e.split(":"), s = e.charAt(0) === "/" ? "/" : "";
    if (t[0] === "xmlns")
      return "";
    t.length === 2 && (e = s + t[1]);
  }
...
const me = new RegExp(`([^\\s=]+)\\s*(=\\s*(['"])([\\s\\S]*?)\\3)?`, "gm");
function Ae(e, t, s) {
  if (!this.options.ignoreAttributes && typeof e == "string") {
    const i = $.getAllMatches(e, me), n = i.length, r = {};
    for (let f = 0; f < n; f++) {
      const u = this.resolveNameSpace(i[f][1]);
      let o = i[f][4], l = this.options.attributeNamePrefix + u;
      if (u.length)
        if (this.options.transformAttributeName && (l = this.options.transformAttributeName(l)), l === "__proto__" && (l = "#__proto__"), o !== void 0) {
          ...
          const a = this.options.attributeValueProcessor(u, o, t);
          a == null ? r[l] = o : typeof a != typeof o || a !== o ? r[l] = a : r[l] = w(
            o,
...
`);
  ...
  for (let r = 0; r < e.length; r++)
    if (e[r] === "<")
      if (e[r + 1] === "/") {
        const u = T(e, ">", r, "Closing Tag is not closed.");
        ...
        const u = T(e, "-->", r + 4, "Comment is not closed.");
        ...
        const u = T(e, "]]>", r, "CDATA is not closed.") - 2, o = e.substring(r + 9, u);
        ...
        let u = O(e, r, this.options.removeNSPrefix), o = u.tagName;
        const l = u.rawTagName;
        let a = u.tagExp, d = u.attrExpPresent, c = u.closeIndex;
        this.options.transformTagName && (o = this.options.transformTagName(o)), s && i && s.tagname !== "!xml" && (i = this.saveTextToParentTag(i, s, n, !1));
        const p = s;
        if (p && this.options.unpairedTags.indexOf(p.tagname) !== -1 && (s = this.tagsNodeStack.pop(), n = n.substring(0, n.lastIndexOf("."))), o !== t.tagname && (n += n ? "." + o : o), this.isItStopNode(this.options.stopNodes, n, o)) {
          let h = "";
          if (a.length > 0 && a.lastIndexOf("/") === a.length - 1)
            ...
            const b = this.readStopNodeData(e, l, c + 1);
            if (!b) throw new Error(`Unexpected end of ${l}`);
            r = b.i, h = b.tagContent;
          }
          const y = new A(o);
          o !== a && d && (y[":@"] = this.buildAttributesMap(a, n, o)), h && (h = this.parseTextData(h, o, n, !0, d, !0, !0)), n = n.substr(0, n.lastIndexOf(".")), y.add(this.options.textNodeName, h), this.addChild(s, y, n);
        } else {
          if (a.length > 0 && a.lastIndexOf("/") === a.length - 1) {
            ...
            const h = new A(o);
            o !== a && d && (h[":@"] = this.buildAttributesMap(a, n, o)), this.addChild(s, h, n), n = n.substr(0, n.lastIndexOf("."));
          } else {
            const h = new A(o);
            this.tagsNodeStack.push(s), o !== a && d && (h[":@"] = this.buildAttributesMap(a, n, o)), this.addChild(s, h, n), s = h;
          }
          i = "", r = c;
        }
      }
...
function Oe(e, t, s) {
  const i = this.options.updateTag(t.tagname, s, t[":@"]);
  ...
  for (; s < e.length; s++)
    if (e[s] === "<")
      if (e[s + 1] === "/") {
        ...
        s = T(e, "?>", s + 1, "StopNode is not closed.");
      else if (e.substr(s + 1, 3) === "!--")
        s = T(e, "-->", s + 3, "StopNode is not closed.");
      else if (e.substr(s + 1, 2) === "![")
        s = T(e, "]]>", s, "StopNode is not closed.") - 2;
      else {
        const r = O(e, s, ">");
...
function w(e, t, s) {
  if (t && typeof e == "string") {
    const i = e.trim();
    return i === "true" ? !0 : i === "false" ? !1 : Ne(e, s);
  } else
    return $.isExist(e) ? e : "";
}
var xe = be, B = {};
  ...
  for (let r = 0; r < e.length; r++) {
    ...
    if (s === void 0 ? o = u : o = s + "." + u, u === t.textNodeName)
      ...
      if (f[u]) {
        let l = k(f[u], t, o);
        const a = Fe(l, t);
        f[":@"] ? _e(l, f[":@"], o, t) : Object.keys(l).length === 1 && l[t.textNodeName] !== void 0 && !t.alwaysCreateTextNode ? l = l[t.textNodeName] : Object.keys(l).length === 0 && (t.alwaysCreateTextNode ? l[t.textNodeName] = "" : l = ""), n[u] !== void 0 && n.hasOwnProperty(u) ? (Array.isArray(n[u]) || (n[u] = [n[u]]), n[u].push(l)) : t.isArray(u, o, a) ? n[u] = [l] : n[u] = l;
      }
    }
  }
...
function Fe(e, t) {
  const { textNodeName: s } = t, i = Object.keys(e).length;
  return !!(i === 0 || i === 1 && (e[s] || typeof e[s] == "boolean" || e[s] === 0));
}
B.prettify = Ve;
const { buildOptions: Le } = v, Be = xe, { prettify: ke } = B, Me = I;
let Xe = class {
  constructor(t) {
    this.externalEntities = {}, this.options = Le(t);
  }
  /**
  ...
  parse(t, s) {
    ...
    return this.options.preserveOrder || n === void 0 ? n : ke(n, this.options);
  }
  /**
...
function Ze(e, t) {
  let s = "";
  return t.format && t.indentBy.length > 0 && (s = Ge), M(e, t, "", s);
}
function M(e, t, s, i) {
  let n = "", r = !1;
  for (let f = 0; f < e.length; f++) {
    ...
    if (s.length === 0 ? l = o : l = `${s}.${o}`, o === t.textNodeName) {
      let h = u[o];
      Ye(l, t) || (h = t.tagValueProcessor(o, h), h = X(h, t)), r && (n += i), n += h, r = !1;
      continue;
      ...
      const h = F(u[":@"], t), y = o === "?xml" ? "" : i;
      let b = u[o][0][t.textNodeName];
      b = b.length !== 0 ? " " + b : "", n += y + `<${o}${b}${h}?>`, r = !0;
      continue;
    }
    let a = i;
    a !== "" && (a += t.indentBy);
    const d = F(u[":@"], t), c = i + `<${o}${d}`, p = M(u[o], t, l, a);
    t.unpairedTags.indexOf(o) !== -1 ? t.suppressUnpairedNode ? n += c + ">" : n += c + "/>" : (!p || p.length === 0) && t.suppressEmptyNode ? n += c + "/>" : p && p.endsWith(">") ? n += c + `>${p}${i}</${o}>` : (n += c + ">", p && i !== "" && (p.includes("/>") || p.includes("</")) ? n += i + t.indentBy + p + i : n += p, n += `</${o}>`), r = !0;
  }
  return n;
}
function Ue(e) {
...
function F(e, t) {
  ...
  if (e && !t.ignoreAttributes)
    for (let i in e) {
      ...
      n = X(n, t), n === !0 && t.suppressBooleanAttributes ? s += ` ${i.substr(t.attributeNamePrefix.length)}` : s += ` ${i.substr(t.attributeNamePrefix.length)}="${n}"`;
    }
  return s;
}
function Ye(e, t) {
  ...
  for (let i in t.stopNodes)
    if (t.stopNodes[i] === e || t.stopNodes[i] === "*." + s) return !0;
  return !1;
}
function X(e, t) {
  if (e && e.length > 0 && t.processEntities)
    for (let s = 0; s < t.entities.length; s++) {
      const i = t.entities[s];
...
function E(e) {
  this.options = Object.assign({}, We, e), this.options.ignoreAttributes || this.options.attributesGroupName ? this.isAttribute = function() {
    return !1;
  } : (this.attrPrefixLen = this.options.attributeNamePrefix.length, this.isAttribute = Qe), this.processTextOrObjNode = ze, this.options.format ? (this.indentate = Ke, this.tagEndChar = `>
`, this.newLine = `
...
E.prototype.build = function(e) {
  return this.options.preserveOrder ? qe(e, this.options) : (Array.isArray(e) && this.options.arrayNodeName && this.options.arrayNodeName.length > 1 && (e = {
    [this.options.arrayNodeName]: e
  }), this.j2x(e, 0).val);
};
E.prototype.j2x = function(e, t) {
  let s = "", i = "";
  for (let n in e)
    if (Object.prototype.hasOwnProperty.call(e, n))
      if (typeof e[n] > "u")
        ...
        const r = Object.keys(e[n]), f = r.length;
        for (let u = 0; u < f; u++)
...
E.prototype.buildObjectNode = function(e, t, s, i) {
  if (e === "")
...
E.prototype.buildTextValNode = function(e, t, s, i) {
...
//# sourceMappingURL=fxp-DtxDeb_e.js.map
