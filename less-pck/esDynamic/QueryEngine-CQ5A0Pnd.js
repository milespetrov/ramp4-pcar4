import { n as ot, a as Z, h as H, j as V, y as re, P as Ze, x as Te, g as U, b as ut, c as ct, S as Se, v as j, t as ht, I as He, d as dt } from "./timeSupport-BajsmNHb.js";
import { d4 as mt, d5 as ft, dy as Re, s as C, cV as we, ba as me, bb as fe, cL as Ye, cM as We, da as ge, K as L, b9 as be, d8 as ne, o6 as Ae, o7 as gt, mQ as Ce, m2 as pt, o8 as _t, ad as ee, kE as pe, lV as Xe, o9 as te, eC as yt, O as le, P as Ne, Q as Tt, dH as Et, oa as oe, bQ as ue, S as It, T as xt, ob as _e, b2 as O, D as Ft, ft as St, fi as Rt, oc as wt, od as bt, oe as At, aT as Ct, cU as Nt, Z as vt, of as v, al as ve, ey as Oe, dm as Ot, og as Pt, bz as $t, bC as Lt, bo as ce, bR as Dt, bw as kt, cZ as Qt, aM as Vt, oh as Pe, aX as Gt, aZ as Mt, aR as Ut, bA as qt, bT as zt } from "./main-n1d2W0Ts.js";
import { O as jt } from "./WhereClause-35G35EJ6.js";
import { t as Bt } from "./QueryEngineCapabilities-Deb54eFC.js";
import { w as Zt } from "./utils-COd2Fnz7.js";
import { c as he, B as Ht, d as $e, p as Le, C as Yt, k as Wt, $ as Xt, E as Jt, P as Kt, U as es, T as ts, v as ss, f as is } from "./utils-qMIyw8_M.js";
...
function ns(l, e, t, i) {
  ...
  return D(l, e, Ee(l, t, { validateStandardized: !0, expressionName: a }), { expressionName: a, query: i }), !0;
}
...
class b {
  ...
  async createQueryResponse() {
    ...
    if (this.query.outStatistics ? e = this.query.outStatistics.some((t) => t.statisticType === "exceedslimit") ? this._createExceedsLimitQueryResponse(this.query) : await this._createStatisticsQueryResponse(this.query) : e = this._createFeatureQueryResponse(this.query), this.query.returnQueryGeometry) {
      ...
      ge(this.query.outSR) && !L(t.spatialReference, this.query.outSR) ? e.queryGeometry = H({ spatialReference: this.query.outSR, ...V(t, t.spatialReference, this.query.outSR) }) : e.queryGeometry = H({ spatialReference: this.query.outSR, ...t });
    }
    return e;
  }
    ...
    for (const d of this.items) {
        ...
        for (let y = 0; y < S.length; y++) {
          ...
          for (let x = 0; x < _; x++, F += a) {
            ...
            if (T.coordsIndex = F, x !== _ - 1) {
              ...
              const w = p;
              hs(p, s, T, R);
              const k = (s.x - w.x) / n, W = (s.y - w.y) / o, N = k * k + W * W;
      ...
      if (e.vertexMode !== "none") {
        ...
        if (e.vertexMode === "all") for (let y = 0; y < F; y += a) {
          ...
          R <= 1 && u.candidates.push(ke(i.getObjectId(d), h(_), Math.sqrt(R)));
        }
          ...
          for (const _ of y) {
            ...
            w <= 1 && u.candidates.push(ke(i.getObjectId(d), h(x), Math.sqrt(w)));
          }
        }
      }
    }
  ...
  async _createStatisticsQueryResponse(e) {
    ...
    for (const F of u ?? []) {
      const { outStatisticFieldName: y, statisticType: _ } = F, x = F, T = _ !== "exceedslimit" ? F.onStatisticField : void 0, R = _ === "percentile_disc" || _ === "percentile_cont", w = _ === "EnvelopeAggregate" || _ === "CentroidAggregate" || _ === "ConvexHullAggregate", k = d && p === 1 && (T === I || E) && _ === "count";
      if (d) {
        if (!r.has(T)) {
          const X = [];
          for (const ae of c) {
  ...
  async _getAggregateGeometry(e, t) {
    const { convexHull: i, union: a } = await import("./geometryEngineJSON-BOcMOp9V.js").then((d) => d.g), { statisticType: s, outStatisticFieldName: r } = e, { featureAdapter: n, spatialReference: o, geometryType: u, hasZ: c, hasM: h } = this, m = t.map((d) => Z(u, c, h, n.getGeometry(d))), f = i(o, m, !0)[0], p = { aggregateGeometries: null, outStatisticFieldName: null };
  ...
  _getStatisticValue(e, t, i, a) {
    ...
    let n = null;
    return n = i?.has(s) ? i.get(s) : ne(this.fieldsIndex.get(s)) || this._isAnyDateField(s) ? $e({ values: t, returnDistinct: a }) : Le({ values: a ? [...new Set(t)] : t, minValue: null, maxValue: null, useSampleStdDev: !0 }), i && i.set(s, n), n[r === "var" ? "variance" : r];
  }
...
function ms(l, e, t) {
  ...
  if (n) {
    ...
    for (const u of i) {
      ...
      } else l.get(c) && h !== "count" && h !== "min" && h !== "max" && D(l, e, [c], { expressionName: `outStatistics with '${h}' statistic type`, allowedFieldTypes: ds, query: t });
    }
  }
}
...
class Ls {
  ...
  async executeQueryForSnapping(e, t) {
    ...
    const u = !L(a.spatialReference, this.spatialReference);
  ...
  async _executeSceneFilterQuery(e, t) {
    ...
    const { outSR: i, returnGeometry: a, returnCentroid: s } = e, r = this.featureStore.featureSpatialReference, n = e.sceneFilter.geometry, o = r == null || L(r, n.spatialReference) ? n : V(n, r);
    if (!o) return null;
    const u = a || s, c = ge(i) && !L(this.spatialReference, i) && u ? async (d) => this._project(d, i) : (d) => d, h = this.featureAdapter, m = await this._reschedule(() => this._searchFeatures(de(o)), t);
  ...
  async _executeGeometryQuery(e, t, i) {
    ...
    if (r === "esriSpatialRelDisjoint") {
      ...
      const _ = t != null ? t.items : await this._reschedule(() => this._getAllFeatures(), i), x = await this._reschedule(async () => {
        const T = await j(r, c, this.geometryType, this.hasZ, this.hasM), R = (k) => !y.has(I.getObjectId(k)) || T(I.getGeometry(k)), w = await this._runSpatialFilter(_, R, i);
    ...
    if (t != null) {
      const y = new zt();
      E = E.filter((_) => Dt(t.items, _, t.items.length, y) >= 0);
    }
    if (!E.length) {
  ...
  async _project(e, t) {
    if (!t || L(this.spatialReference, t)) return e;
    const i = this.featureAdapter;
    let a;
    try {
      const r = await this._getFullExtent();
      a = kt(this.spatialReference, t, r);
    } catch {
    }
...
//# sourceMappingURL=QueryEngine-CQ5A0Pnd.js.map
