import { aR as A, aS as R, aT as $, ar as j, aU as T, aV as J, aW as N, aM as U, aX as V, aY as b, aZ as W, a_ as Y, a$ as Z, b0 as p } from "./main-n1d2W0Ts.js";
function F(i) {
  return k(i, !0);
}
function G(i) {
...
function k(i, s) {
  ...
  const t = i.spatialReference, n = R(t), e = $(i) ? i.toJSON() : i;
  if (!n) return e;
  const h = j(t) ? 102100 : 4326, u = T[h].maxX, a = T[h].minX;
  if (J(e)) return S(e, u, a);
  if (N(e)) return e.points = e.points.map((o) => S(o, u, a)), e;
  if (U(e)) return q(e, n);
  if (V(e) || b(e)) {
    const o = W(B, e), r = { xmin: o[0], ymin: o[1], xmax: o[2], ymax: o[3] }, _ = p(r.xmin, a) * (2 * u), x = _ === 0 ? e : Y(e, _);
    return r.xmin += _, r.xmax += _, r.xmax > u ? L(x, u, s) : r.xmin < a ? L(x, a, s) : x;
  }
  return e;
}
function q(i, s) {
  if (!s) return i;
  const t = z(i, s).map((n) => n.extent);
...
function z(i, s) {
  const t = [], { ymin: n, ymax: e, xmin: h, xmax: u } = i, a = i.xmax - i.xmin, [o, r] = s.valid, { x: _, frameId: x } = w(i.xmin, s), { x: l, frameId: c } = w(i.xmax, s), X = _ === l && a > 0;
  if (a > 2 * r) {
    const v = { xmin: h < u ? _ : l, ymin: n, xmax: r, ymax: e }, O = { xmin: o, ymin: n, xmax: h < u ? l : _, ymax: e }, C = { xmin: 0, ymin: n, xmax: r, ymax: e }, P = { xmin: o, ymin: n, xmax: 0, ymax: e }, f = [], y = [];
    I(v, C) && f.push(x), I(v, P) && y.push(x), I(O, C) && f.push(c), I(O, P) && y.push(c);
    for (let d = x + 1; d < c; d++) f.push(d), y.push(d);
    t.push(new m(v, [x]), new m(O, [c]), new m(C, f), new m(P, y));
  } else _ > l || X ? t.push(new m({ xmin: _, ymin: n, xmax: r, ymax: e }, [x]), new m({ xmin: o, ymin: n, xmax: l, ymax: e }, [c])) : t.push(new m({ xmin: _, ymin: n, xmax: l, ymax: e }, [x]));
  return t;
}
function w(i, s) {
...
function L(i, s, t = !0) {
  const n = !b(i);
  if (n && Z(i), t)
    return new D().cut(i, s);
  const e = n ? i.rings : i.paths, h = n ? 4 : 2, u = e.length, a = -2 * s;
  for (let o = 0; o < u; o++) {
    const r = e[o];
    if (r && r.length >= h) {
      const _ = [];
      for (const x of r) _.push([x[0] + a, x[1]]);
      e.push(_);
    }
  }
...
class D {
  ...
  cut(s, t) {
    ...
    for (const h of n) {
      ...
      let u = !0;
      for (const a of h) u ? (this.moveTo(a), u = !1) : this.lineTo(a);
...
//# sourceMappingURL=normalizeUtilsSync-D4n7itZU.js.map
