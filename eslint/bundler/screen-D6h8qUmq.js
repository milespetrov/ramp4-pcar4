...
const ot = { class: "flex justify-end mb-20" }, st = ["disabled", "animation"], nt = { class: "button-text" }, rt = /* @__PURE__ */ ue({
  __name: "form-footer",
  props: {
    animation: {
      type: Boolean,
  ...
  setup(s) {
    ...
    return (h, p) => (c(), y("div", ot, [
      ...
      r("button", {
        ...
        r("span", nt, g(o(v)("wizard.step.continue")), 1)
      ], 8, st)
    ]));
  }
  ...
  setup(s, { expose: v, emit: l }) {
    const w = xe("iApi"), { t: k } = ke(), h = l, p = s, $ = m(), _ = le("textInput"), z = le("selectInput"), B = le("urlInput");
    v({ selectInput: z, textInput: _, urlInput: B });
    const E = m(!1), W = m(!1), P = m(!1), ae = m(!1), q = m([...p.selectedValues]), G = m("value-label"), Q = m("option-label"), C = m(void 0), I = m(null), j = ie([]);
    if (p.defaultOption && p.modelValue === "" && p.options.length) {
      let n = p.options[0].value;
      if (p.name === "latField") {
        const u = new RegExp(/^(y|lat.*)$/i);
        n = p.options.find((N) => u.test(N.label))?.value || n;
      } else if (p.name === "longField") {
        const u = new RegExp(/^(x|long.*)$/i);
        n = p.options.find((N) => u.test(N.label))?.value || n;
      }
      h("update:modelValue", n);
    }
    const X = (n) => {
      n.trim() !== "" ? E.value = !0 : (E.value = !1, w.updateAlert(k("wizard.configure.name.error.required")));
    }, oe = (n) => {
      let u;
      try {
        u = new URL(n);
      ...
      u.protocol === "http:" || u.protocol === "https:" ? E.value = !0 : E.value = !1;
    }, de = (n) => {
      h("upload", n.target.files[0]), n.target.value = "";
    }, ee = (n) => {
      oe(n.target.value), h("link", n.target.value, E), W.value = !1;
    }, x = (n, u) => {
      h(n ? "select" : "update:modelValue", u.target.value);
    }, se = (n) => {
      h("nested", n.target.checked);
    }, ne = (n) => {
      X(n.target.value), h("link", n.target.value, E), P.value = !1;
    }, V = () => {
      h("select", p.sublayerOptions(q.value)), ae.value = !(q.value && q.value.length > 0);
    }, Y = (n) => n.length > 5 ? `${n.slice(0, 5)}...` : n;
    function F() {
    ...
    const a = () => {
      const n = w.$vApp.$el.querySelector(".vue-treeselect__menu"), u = n ? n.clientHeight : 0, M = w.$vApp.$el.querySelector(".vue-treeselect__control"), N = M ? M.clientHeight : 0;
      $.value.style.height = `${u + N + 30}px`;
    };
    j.push(
      Se(I, (n) => {
        n && ce();
    ...
    const ce = () => {
      if (I.value) {
        const n = I.value.querySelector('input[type="text"]');
        n && n.setAttribute("aria-label", k("wizard.configure.sublayers.select"));
      }
    }, ve = () => {
      z.value._tippy.hide();
    }, K = (n) => {
      n.key === "Tab" && z.value?.matches(":focus") && navigator.userAgent.includes("Firefox") ? z.value._tippy.show() : z.value._tippy.hide();
    };
    return ze(() => {
      z.value?.addEventListener("blur", ve), z.value?.addEventListener("keyup", K), p.step === 2 && p.step === p.activeStep && h("focusElement");
    }), me(() => {
      C.value.disconnect(), j.forEach((n) => n()), z.value?.removeEventListener("blur", ve), z.value?.removeEventListener("keyup", K);
    }), (n, u) => {
      const M = we("truncate"), N = we("tippy");
      return c(), y("div", {
        ...
        s.type === "file" ? (c(), y("div", it, [
            ...
            s.multiple ? (c(), y("div", St, [
              r("div", {
                ...
                O(o(at), {
                  ...
                  [Q.value]: R(({ node: b, labelClassName: pe }) => [
                    fe((c(), y("label", {
                      class: re(pe)
                    }, [
                      Ee(g(b.label), 1)
              ...
              s.validation && ae.value ? (c(), y("div", xt, g(s.validationMessages?.required), 1)) : A("", !0)
            ])) : (c(), y("div", zt, [
              fe((c(), y("select", {
                class: re(["block border-solid border-gray-300 w-full p-3 overflow-y-auto", s.size && "configure-select"]),
                size: s.size,
                value: s.modelValue,
          ...
          r("label", $t, g(s.label), 1)
        ])) : (c(), y("div", It, [
          r("label", Ot, g(s.label), 1),
          r("div", Tt, [
            r("input", {
              class: re(["border-solid border-gray-300 p-3 w-full", { "error-border": !E.value && !s.modelValue }]),
              type: "text",
              value: s.modelValue,
              "aria-label": p.ariaLabel,
...
}), U = /* @__PURE__ */ be(Rt, [["__scopeId", "data-v-e14cab8b"]]), _t = { class: "step relative flex flex-col px-12" }, Nt = { class: "stepper-header flex pb-24" }, Ut = {
  key: 1,
  ...
  setup(s, { emit: v }) {
    const l = xe("stepper"), w = m(), k = v, h = m(-1), p = (z) => {
      _() || (z.stopPropagation(), k("focusPanel"), k("focusFirstElement"));
    };
    ze(() => {
    ...
    const $ = () => l.activeIndex > h.value, _ = () => l.activeIndex === h.value;
    return (z, B) => {
      const E = we("truncate");
      return c(), y("div", _t, [
        r("div", Nt, [
          $() ? (c(), y("div", Ut, B[0] || (B[0] = [
            ...
            key: 0,
            class: re(["w-24 h-24 bg-gray-500 rounded-full flex justify-center items-center text-white text-xs font-semibold", { "bg-blue-500": _ }])
          }, g(h.value + 1), 3)),
          r("div", Bt, [
            r("div", qt, g(s.title), 1),
            fe((c(), y("div", Ht, [
              Ee(g(s.summary), 1)
            ])), [
              [$e, !_()],
  ...
  setup(s) {
    const v = s, l = D(() => v.activeStep), w = ie([]), k = ie({
  ...
  setup(s) {
    const v = lt(), { t: l } = ke(), w = xe("iApi"), k = m(), h = m([]), p = D(() => v.layerSource), $ = D(() => v.currStep), _ = m(), z = m(), B = m(0), E = m(!1), W = m(), P = le("stepOneStart"), ae = le("stepTwoStart"), q = le("stepThreeStart"), G = m(!1), Q = m(!1), C = m(!1), I = m(!1), j = m(!1), X = m(!1), oe = m(!0), de = m(""), ee = m(""), x = m([]), se = ie([
      {
        value: S.FEATURE,
    ...
    const u = async (e) => {
      ...
      t.onload = (d) => {
        Y.value = t.result, V.value = e.name, M(d);
      }, t.readAsArrayBuffer(e);
    }, M = (e) => {
      e?.preventDefault(), F.value = p.value.guessFormatFromURL(V.value), v.goToStep(J.FORMAT);
    }, N = async (e) => {
      e?.preventDefault(), E.value = !0, Q.value = !1, j.value = !0, v.goToStep(J.CONFIGURE), ee.value = Z() ? ne.find((d) => d.value === F.value)?.label : se.find((d) => d.value === F.value)?.label;
      try {
      ...
      Ve(), I.value = !(a.value.configOptions.includes("sublayers") || !a.value.config.name), E.value = !1, j.value = !1;
    }, b = async (e) => {
      e?.preventDefault();
      ...
      if (v.nested = e, x.value = [], B.value += 1, F.value === S.MAPIMAGE) {
      ...
      const f = (i) => {
        const T = d.get(i);
        return T ? T.every((te) => d.has(te) ? f(te) : t.has(te)) : !1;
      }, L = (i) => {
        if (f(i))
          x.value.push(i);
        else {
      ...
      _.value = a.value?.config.colour ?? "#" + Math.floor(Math.random() * 16777215).toString(16).padStart(6, "0");
      do
      ...
      C.value = !1, v.goToStep(0), n(P);
    }, Pe = () => {
      E.value = !1, G.value = !1, Q.value = !1, C.value = !!V.value, j.value = !1, v.goToStep(0), ee.value = "";
    }, Ge = () => {
              ...
              O(he, {
                ...
                default: R(() => [
                  r("form", {
                    ...
                    a.value?.configOptions.includes("name") ? (c(), H(U, {
                      key: 0,
                      type: "text",
                      name: "name",
                    ...
                    a.value?.configOptions.includes("nameField") ? (c(), H(U, {
                      key: 1,
                      type: "select",
                      name: "nameField",
                    ...
                    a.value?.configOptions.includes("tooltipField") ? (c(), H(U, {
                      key: 2,
                      type: "select",
                      name: "tooltipField",
                    ...
                    a.value?.configOptions.includes("latField") ? (c(), H(U, {
                      key: 3,
                      type: "select",
                      name: "latField",
                    ...
                    a.value?.configOptions.includes("longField") ? (c(), H(U, {
                      key: 4,
                      type: "select",
                      name: "longField",
...
}), _l = /* @__PURE__ */ be(Xt, [["__scopeId", "data-v-cdc2519a"]]);
export {
  _l as default
};
