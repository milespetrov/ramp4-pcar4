import{em as A,fM as E,en as R,eB as j,r as T,eq as _,a as C,eG as V,ed as X,eF as F,eH as q,er as U,eu as B,ev as k,cZ as n,eI as D,ey as c,ez as p,cX as d,cY as h}from"./main-CZM6prq-.js";import{m as G}from"./marked.esm-DgqJBp_S.js";const Y=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Z={key:0},J={class:"flex justify-center"},K={key:0,class:"flex flex-col justify-center text-center"},Q=["innerHTML"],W=["innerHTML"],ee={key:3,class:"flex flex-col justify-center text-center"},te={class:"text-xl mt-20"},ae={key:1,class:"p-5"},le=A({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(r){const s=E(),{t:o}=R(),u=j("iApi"),I=T(),l=r,f=_(()=>s.status),v=_(()=>s.response),b=T(!1),x=C({}),y=C([]),$=C([]);V(()=>{S(),y.push(u.event.on(X.LAYER_REMOVE,e=>{l.payload.layer?.uid===e.uid&&l.panel.close()})),$.push(F(()=>l.payload.layer.uid,(e,t)=>{e!==t&&S()}))}),q(()=>{y.forEach(e=>u.event.off(e)),$.forEach(e=>e())});const S=()=>{b.value=l.payload.layer!==void 0&&!l.payload.layer.isRemoved,l.payload.type==="xml"?M(l.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(g(`${e.firstElementChild.outerHTML}`)),(l.payload.catalogueUrl||l.payload.url)&&t.appendChild(g(`<h5 class="text-xl font-bold mb-3">${o("metadata.xslt.metadata")}</h5>`)),l.payload.catalogueUrl&&t.appendChild(g(`<p><a style="color: blue;" href="${l.payload.catalogueUrl}" target="_blank">${o("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(g(`<p><a style="color: blue;" href="${l.payload.url}" target="_blank">${o("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):l.payload.type==="html"?w(l.payload.url).then(e=>{s.status=e.status,s.response=e.response}):l.payload.type==="md"&&w(l.payload.url).then(e=>{s.status=e.status,s.response=G(e.response)})},M=(e,t)=>{let a=u.language==="en"?Y:z;return a=a.replace(/\{\{([\w.]+)\}\}/g,(i,m)=>o(m)),x[e]?Promise.resolve(L(x[e],a,t)):w(e).then(i=>(x[e]=i.response,L(x[e],a,t)))},L=(e,t,a)=>{let i=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,O=new DOMParser,N=O.parseFromString(e,"text/xml"),H=O.parseFromString(t,"text/xml");m.importStylesheet(H),a&&a.forEach(P=>m.setParameter("",P.key,P.value||"")),i=m.transformToFragment(N,document)}return i},w=e=>new Promise(t=>{const a=new XMLHttpRequest;a.open("GET",e,!0),a.responseType="text",a.onload=()=>{a.status===200?t({status:"success",response:a.response}):t({status:"error",response:"Could not load results from remote service."})},a.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},a.send()});function g(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const a=U("panel-screen");return n(),B(a,{panel:r.panel,ref_key:"el",ref:I},{header:k(()=>[D(c(p(o)("metadata.title"))+": "+c(r.payload.layerName),1)]),content:k(()=>[b.value?(n(),d("div",Z,[h("div",J,[f.value=="loading"?(n(),d("div",K,c(p(o)("metadata.loading")),1)):r.payload.type==="xml"&&f.value=="success"?(n(),d("div",{key:1,innerHTML:v.value,class:"flex flex-col justify-center xml-content"},null,8,Q)):(r.payload.type==="html"||r.payload.type==="md")&&f.value=="success"?(n(),d("div",{key:2,innerHTML:v.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,W)):(n(),d("div",ee,[t[0]||(t[0]=h("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),h("span",te,c(p(o)("metadata.error")),1)]))])])):(n(),d("div",ae,[h("span",null,c(p(o)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{le as default};
